        ...
        <v-card-title primary-title class="pt-2 pb-0" v-show="!custom_title">
            <v-layout row wrap>
                ...
                <v-flex xs3 class="pr-4">
                    <!-- <div class="orange lighten-2 fill-height d-flex align-center text-xs-center"><h3><a href="./due/" class="white--text">TAGIHAN JATUH TEMPO</a></h3></div> -->
                </v-flex>
                <v-flex xs1 pr-1>
                    <common-datepicker
                        label="Dari Tanggal"
            ...
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    ...
                    <td class="text-xs-right pa-2" :class="bg_proforma(props.item)" @click="select(props.item)" v-show="!is_sales">
                        <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(props.item.invoice_grand_total)) }}</b><br>
                    </td>
                    <td class="text-xs-right pa-2 red--text" :class="bg_proforma(props.item)" @click="select(props.item)" v-show="!is_sales">
                        <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(props.item.retur_nominal)) }}</b><br>
                    </td>
                    <!-- <td class="text-xs-left pa-2" :class="bg_proforma(props.item)" @click="select(props.item)" v-show="!is_sales">
...
module.exports = {
    ...
    computed : {
        ...
        headerStats () {
            return this.$store.state.invoice.headerStats
        }
    },
    ...
    mounted () {
        ...
        this.$store.dispatch('invoice/searchHeaderStats')
    }
}
</script>