        ...
        <v-card-title primary-title class="pt-2 pb-0" v-show="!custom_title">
            <v-layout row wrap>
                ...
                <v-flex xs2 class="pr-4">
                    <div class="orange lighten-2 fill-height d-flex align-center text-xs-center"><h3><a href="../" class="white--text">SEMUA TAGIHAN</a></h3></div>
                </v-flex>
                <v-flex xs1 pr-1>
                    <common-datepicker
            ...
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    ...
                    <td class="text-xs-left pa-2" :class="bg_proforma(props.item)" @click="select(props.item)">
                        <div>{{ props.item.invoice_due_date }}</div>
                        <div v-show="props.item.invoice_past_due>0" class="caption red--text"><i>Lewat {{ props.item.invoice_past_due }} hari</i></div>
                    </td>
                    ...
                    <td class="text-xs-left pa-2" :class="bg_proforma(props.item)" @click="select(props.item)" v-show="!is_sales">
                        ...
                        <div>
                            <div class="caption green--text"><i>— {{props.item.sales.staff_name}}</i></div>
                        </div>
                    </td> 
                    ...
                    <td class="text-xs-right pa-2" :class="bg_proforma(props.item)" @click="select(props.item)" v-show="!is_sales">
                        ...
                        <div v-show="props.item.retur_nominal>0" class="red--text">
                            <i><span class="grey--text caption">Rp</span> {{ one_money(Math.round(props.item.retur_nominal)) }}</i>
                        </div>
                    </td>
                    <td class="text-xs-right pa-2 primary--text" :class="bg_proforma(props.item)" @click="select(props.item)" v-show="!is_sales">
                        <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(props.item.invoice_paid)) }}</b><br>
                    </td>

                    <td class="text-xs-right pa-2 red--text" :class="bg_proforma(props.item)" @click="select(props.item)" v-show="!is_sales">
                        <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(props.item.invoice_unpaid)) }}</b><br>
                    </td>
...
module.exports = {
    ...
    computed : {
        ...
        due_date : {
            get () { return this.$store.state.invoice.due_date },
            set (v) { this.$store.commit('invoice/set_common', ['due_date', v]) }
        },
        ...
        staffs () {
            return this.$store.state.invoice.staffs
        },

        selected_staff : {
            get () { return this.$store.state.invoice.selected_staff },
            set (v) { 
                this.$store.commit('invoice/set_object', ['selected_staff', v])
                this.search()
            }
        }
    },
    ...
    methods : {
        ...
        search () {
            return this.$store.dispatch('invoice/search_due')
        },
        ...
        change_due_date(x) {
            this.$store.commit('invoice/set_common', ['due_date', x.new_date])
