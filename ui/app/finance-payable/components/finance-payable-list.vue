                ...
                <v-flex xs3 class="text-xs-right" pl-2>
                    ...
                    >
                        <template v-slot:append-outer>
                            ...
                            <v-btn color="orange" class="ma-0 ml-2 btn-icon" @click="printMe" v-show="!is_sales" dark>
                                <v-icon>print</v-icon>
                            </v-btn>  
                        </template>
                    </v-text-field>
            ...
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    <td class="text-xs-left pa-1" :class="bg_proforma(props.item)" @click="select(props.item)" style="writing-mode:tb-rl;height:auto;transform:scale(-1)">
                        <span class="orange white--text px-2 py-3 d-block" v-if="props.item.invoice_unpaid==0">LUNAS</span>
                        <span class="success pa-2 py-3 d-block white--text" v-if="props.item.invoice_unpaid>0&&props.item.invoice_paid>0">SEBAGIAN</span>
                        <span class="grey white--text pa-2 py-3 d-block" v-if="props.item.invoice_paid==0">BARU</span>
                    </td>
                    ...
                    <td class="text-xs-left pa-2" :class="bg_proforma(props.item)" @click="select(props.item)" v-show="!is_sales">
                        ...
                        <div v-show="props.item.memo_total>0" class="red pa-1 mt-1">
                            <a :href="'../../finance-memo/list?search='+props.item.customer_name" class="white--text" target="_blank">
                                <i>Tersedia memo kredit : Rp <b>{{ one_money(props.item.memo_total) }}</b></i></a>
                            <div><a class="white--text" v-for="(m, n) in props.item.memos" :key="n" :href="'../../sales-retur/new?id='+m.retur_id" target="_blank">{{ m.memo_invoice }}</a> &nbsp; </div>
                            </div>
                    </td> 
                    ...
                    <td class="text-xs-left pa-2" :class="bg_proforma(props.item)" @click="select(props.item)" v-show="!is_sales">
                        <v-layout row wrap v-for="(p,i) in props.item.payments" :key="i">
                            <v-flex>
                                <a :href="'../pay/'+p.pay_id"  @click="edit_payment(props.item, p.pay_id, $event)" >
                                    {{p.pay_number}}, {{p.pay_date}}
                                </a>
                            </v-flex>
                            <!-- <v-flex class="text-xs-right">
                                <span class="grey--text caption">Rp</span> <b class="cyan--text">{{ one_money(Math.round(props.item.invoice_paid)) }}</b>    
                            </v-flex> -->
                        </v-layout>
                    </td>
                    <td class="text-xs-left pa-2" :class="bg_proforma(props.item)" @click="select(props.item)" v-show="!is_sales">
                    ...
                    <td class="text-xs-right pa-2" :class="bg_proforma(props.item)" @click="select(props.item)" v-show="!is_sales">
                        <span class="grey--text caption">Rp</span> <b class="red--text">{{ one_money(Math.round(props.item.invoice_unpaid)) }}</b>
                    </td>
                    ...
                    <td class="text-xs-center pa-0" :class="bg_proforma(props.item)" @click="select(props.item)">
                        <div class="row">
                            <div class="col-12">
                                ...
                                <v-btn color="cyan" class="btn-icon ma-0" small @click="pay(props.item)" 
                                    :dark="props.item.invoice_unpaid>0" 
                                    :disabled="props.item.invoice_unpaid==0"
                                    title="Penerimaan Piutang"><v-icon>attach_money</v-icon></v-btn>
...
module.exports = {
    ...
    methods : {
        ...
        setObject (x, y) {
            this.$store.commit("payable/set_object", [x, y])
        },

        setObjectPay (x, y) {
            this.$store.commit("payablePay/set_object", [x, y])
        },
        ...
        pay (x) {
            this.select(x)
            this.setObjectPay("invoice_id", x.invoice_id)
            this.setObjectPay("invoice_number", x.invoice_number)
            this.setObjectPay("invoice_date", x.invoice_date)
            this.setObjectPay("invoice_total", x.invoice_total)
            this.setObjectPay("invoice_grand_total", x.invoice_grand_total)
            this.setObjectPay("invoice_paid", x.invoice_paid)
            this.setObjectPay("invoice_unpaid", x.invoice_unpaid)
            this.setObjectPay("dialog_pay", true)
            this.setObjectPay("edit", false)

            this.setObjectPay("pay_amount", 0)
            this.setObjectPay("pay_date", this.$store.state.payable.current_date.split('-').reverse().join('-'))
            this.setObjectPay("pay_note", '')
            this.setObjectPay("pay_id", 0)

            this.setObjectPay("selected_customer", {customer_id:x.customer_id,customer_name:x.customer_name})
            ...
            this.setObjectPay("selected_memos", [this.$store.state.payablePay.default_memo])
        ...
        edit_payment(x, y, e) {
            ...
            this.$store.dispatch("payablePay/search_pay_id").then(res => {
                this.setObjectPay("invoice_id", res.invoice_id)
                this.setObjectPay("invoice_number", res.invoice_number) 
                this.setObjectPay("invoice_date", res.invoice_date) 
                this.setObjectPay("invoice_grand_total", res.invoice_grand_total) 
                this.setObjectPay("invoice_unpaid", x.invoice_unpaid) 
                this.setObjectPay("invoice_paid", x.invoice_paid) 
                ...
                this.setObjectPay("selected_customer", res.customer)
        ...
        printMe () {
            this.$store.dispatch("invoice/collect").then((x) => {
                window.open(this.$store.state.payable.URL + 'report/one_fin_011/excel?' + x)
            })
        }
    },
