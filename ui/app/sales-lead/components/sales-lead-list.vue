            ...
            <table class="v-datatable v-table theme--light" style="table-layout:fixed">
                <thead>
                    <tr>
                        ...
                        <th role="columnheader" scope="col" width="6%" aria-label="CATATAN: Not sorted." aria-sort="none" class="column text-xs-right pa-2 zalfa-bg-purple lighten-3 white--text" v-for="(c, n) in categories" :key="'c'+n" style="overflow-x:hidden">
                            {{c.category_name.toUpperCase()}}
                        </th>

                        <th role="columnheader" scope="col" width="10%" aria-label="ACTION: Not sorted." aria-sort="none" class="column text-xs-center pa-2 zalfa-bg-purple lighten-3 white--text">ACTION</th>
                    </tr>
                ...
                <tbody>
                    <template v-for="(l, n) in leads">
                        ...
                        <tr :key="'x'+n">
                            <td class="text-xs-left py-1 px-2" @click="select(l)" v-show="!is_sales" colspan="11">
                                <i>{{l.lead_note}}</i></td> 
                        </tr>
                    </template>
                </tbody>
...
module.exports = {
    ...
    methods : {
        ...
        add () {
            ...
            // SET ZERO
            let values = {}
            for (let ctg of this.$store.state.lead_new.categories)
