        ...
        <v-card-text primary-title class="pt-2 pb-0">
            <v-layout row wrap>
                <v-flex xs12>
                    <v-layout row wrap>
                        <v-flex xs9>
                            <h3 class="display-1 font-weight-light zalfa-text-title">PRODUK PARETO BERDASARKAN NOMINAL</h3> 
                                ...
                                <v-flex xs12 v-for="(i, n) in items" :key="n">
                                    <v-layout row wrap>
                                        <v-flex xs6 class="text-xs-right pr-3 align-center d-flex justify-end">
                                            <div>{{ i.item_name }} (<span class="font-weight-bold orange--text px-1"><span class="font-weight-thin">Rp</span> {{ one_money(i.item_total) }}</span>)</div>
                                        </v-flex>
                                        <v-flex xs5>
...
module.exports = {
    components : {
        ...
        'common-datepicker' : httpVueLoader('../../common/components/common-datepicker.vue?t=123'),
    ...
    computed : {
        items () {
            return this.$store.state.inventory.items2
        },
        ...
        edate : {
            get () { return this.$store.state.inventory.edate2 },
            set (v) { this.$store.commit('inventory/set_object', ['edate2', v]) }
        },

        sdate : {
            get () { return this.$store.state.inventory.sdate2 },
            set (v) { this.$store.commit('inventory/set_object', ['sdate2', v]) }
        },
    ...
    methods : {
        ...
        select (x) {
            this.$store.commit('inventory/set_object', ['selected_item2', x])
        },
