...
mounted: function() {
    ...
    this.$store.commit('transfer_new/set_object', ['transfer_id', <?=$_GET['id'];?>])
    this.$store.dispatch('transfer_new/search_id').then((a) => {
      

      // this.$store.dispatch('receive_new/search_vendor').then((x) => {
         this.$store.dispatch('transfer_new/search_warehouse').then((y) => {
...
methods : {
   ...
   components: {
        ...
        "new-form" : httpVueLoader("../components/iv-item-transfer-new-form.vue?t=<?=date('YmdHis')?>"),
