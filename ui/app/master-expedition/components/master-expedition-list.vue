        ...
        <v-card-text class="pt-2">
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    <td class="text-xs-left pa-2" @click="select(props.item)">{{ props.item.expedition_name }}</td>
                    <td class="text-xs-left pa-2 border-left" @click="select(props.item)">{{ props.item.expedition_address }}</td>
                    <td class="text-xs-left pa-2 border-left" @click="select(props.item)">{{ props.item.expedition_destination }}</td>
                    <td class="text-xs-left pa-2 border-left border-left" @click="select(props.item)">{{ props.item.expedition_website }}</td>
                    <td class="text-xs-left pa-2" @click="select(props.item)">{{ props.item.expedition_phone1 }} 
                        <span v-show="props.item.expedition_phone2!=''&&props.item.expedition_phone2"> / </span>{{ props.item.expedition_phone2 }}</td>
...
module.exports = {
    ...
    computed : {
        ...
        expedition_id () {
            return this.$store.state.expedition.selected_expedition.expedition_id
        },
    ...
    methods : {
        ...
        add () {
            ...
            for (let x of ['name', 'code', 'address', 'phone1', 'phone2', 'website',
                    'note', 'destination'])
            this.$store.commit('expedition_new/set_object', ['expedition_'+x, ''])
        ...
        edit (x) {
            ...
            for (let x of ['name', 'code', 'address', 'phone1', 'phone2', 'website',
                    'note', 'destination'])
            this.$store.commit('expedition_new/set_object', ['expedition_'+x, sc['expedition_'+x]])
            this.$store.commit('expedition_new/set_dialog_new', true)
        },
        ...
        confirm_del (x) {
            this.$store.dispatch('expedition/del')
        },
