...
mounted: function() {
    ...
    this.$store.dispatch('accountreport/searchAccount').then((x) => {
      this.$store.dispatch('accountreport/searchAccountGroup').then((y) => {
         this.$store.dispatch('accountreport/searchGroup')
      })
    })
...
methods : {
   ...
   components: {
        ...
        "lists" : httpVueLoader("./components/master-account-report-list.vue?t=<?=date('YmdHis');?>")
      }
    })
    </script>
    <style>
