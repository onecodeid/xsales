    ...
    <v-card>
        ...
        <v-card-text>
            <v-layout row wrap>
                ...
                <v-flex xs3 pr-4 class="d-flex align-end">
                    <v-autocomplete
                        :items="items"
...
module.exports = {
    ...
    methods : {
        __c (a,b) { this.$store.commit("retur/set_object", [a,b]) },
        __d (a) { this.$store.dispatch("retur/" + a) },
        ...
        searchInvoice () {
            if (!!this.selected_customer && !!this.selectedItem) {
                ...
                // this.$store.dispatch('return/search')
                this.__d("searchInvoiceByItem")
    ...
    mounted () {
        ...
        if (id) {
            ...
            this.$store.dispatch('retur/search_id').then(res => {
                ...
                this.setObject('details', res.items)

                this.$store.dispatch('retur/search_customer').then((d) => {
                    for (let c of d.records)
                        if (c.customer_id == res.customer_id)
                            this.__c('selected_customer', c)

                    this.$store.dispatch('retur/search_item').then((e) => {
                        for (let ee of e.records)
                            if (ee.item_id == res.retur_item)
                                this.__c('selectedItem', ee)
                    })
                })
            })
            ...
            this.$store.dispatch('retur/search_item')
