            ...
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    <td class="text-xs-left pa-1" @click="select(props.item)" style="writing-mode:tb-rl;height:auto;transform:scale(-1)">
                        <span class="cyan pa-2 py-3 d-block white--text" v-if="props.item.invoice_id!=0">RETUR</span>
                        <span class="orange pa-2 py-3 d-block white--text" v-if="props.item.invoice_id==0">MEMO</span>
                    ...
                    <td class="text-xs-left pa-2" @click="select(props.item)">
                        <div class="cyan--text">{{ props.item.customer_name }}</div>
                        <div class="caption" v-show="invoice_id!=0">{{ props.item.invoice_number }}</div>
                        <div class="caption" v-show="invoice_id==0">{{ props.item.account_name }}</div>
                    </td> 
                    <td class="text-xs-left pa-2" @click="select(props.item)">
                        <v-layout row wrap>
                    ...
                    <td class="text-xs-right pa-2" @click="select(props.item)">
                        <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(props.item.memo_used+props.item.memo_refunded)) }}</b>
                    </td>
                    <td class="text-xs-center pa-0" @click="select(props.item)">
                        ...
                        <v-btn color="primary" class="btn-icon ma-0" small title="Ubah Memo" :disabled="props.item.invoice_id!=0||props.item.memu_used>0">
                        ...
                        <v-btn color="red" class="btn-icon ma-0" small title="Hapus transaksi" @click="del(props.item)" 
                            :dark="props.item.invoice_id==0&&(props.item.memo_used==0&&props.item.memo_refunded==0)" 
                            :disabled="props.item.invoice_id!=0||props.item.memu_used>0||props.item.memo_refunded>0">
...
module.exports = {
    ...
    computed : {
        ...
        invoice_id () {
            return this.$store.state.memoNew.invoice_id
        },
    ...
    methods : {
        ...
        add (e) {
            ...
            for (let c of ["memo_id", "memo_amount", "memo_disc", "memo_discrp", "invoice_id"]) { this.setNewObject(c, 0) }
