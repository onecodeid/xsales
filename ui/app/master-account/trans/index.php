...
mounted: function() {
   ...
   <?php if (isset($_GET['id'])) { ?>
      this.$store.commit("accountTrans/set_object", ["account_id", <?=$_GET['id'];?>])
      this.$store.dispatch("accountTrans/search")
      this.$store.dispatch('account/search').then((x) => {
         for (let y of x.records) if (y.account_id == <?=$_GET['id'];?>) {
            document.title += ' ' + y.account_name
            this.$store.commit('accountTrans/set_object', ['account', y])
         }
      })
   <?php } ?>
...
methods : {
   ...
   components: {
        ...
        "trans" : httpVueLoader("../components/master-account-trans.vue<?php echo $ts ?>")
      }
    })
    </script>
    <style>
