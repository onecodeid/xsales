...
module.exports = {
    ...
    computed : {
        dialog : {
            get () { return this.$store.state.assembly_new.dialog_item },
            set (v) { this.$store.commit('assembly_new/set_common', ['dialog_item', v]) }
        },
    ...
    methods : {
        select (x) {
            this.$store.commit('assembly_new/set_selected_item_av', x)
        },

        pick (x) {
            this.select(x)
            let items = this.$store.state.assembly_new.items
            ...
            this.$store.commit('assembly_new/set_items', items)
            // this.dialog = false
        },

        is_selected (item) {
            let itm = this.$store.state.assembly_new.items
            for (let i of itm) {
