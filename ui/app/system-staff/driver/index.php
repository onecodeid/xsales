         ...
         <v-app id="oneApp"  v-if="one_token">
            ...
            <!-- <common-notification></common-notification> -->
               <v-container pt-1 pl-1 pr-1 fluid>
                  <v-layout row>
                     <v-flex xs12>
                        <system-staff-list></system-staff-list>
                     </v-flex>
                  </v-layout>
               </v-container>
            </v-content>
            ...
            <system-staff-new></system-staff-new>
         </v-app>
      </div>
...
mounted: function() {
    if (!this.$store.state.system.one_token)
      ...
      this.$store.commit('staff/set_object', ['pos_code', 'POS.TRANSPORTER'])
    this.$store.dispatch('system/search_menu_group')
    this.$store.dispatch('staff/search_driver')
},
computed : {
    one_token () {
...
methods : {
   ...
   components: {
        ...
        "system-staff-list" : httpVueLoader("../components/system-staff-list.vue"),
        "system-staff-new" : httpVueLoader("../components/system-staff-new.vue")
      }
    })
    </script>
    <style>
