         ...
         <v-app id="oneApp"  v-if="one_token">
            ...
            <!-- <common-notification></common-notification> -->
               <v-container pt-1 pl-1 pr-1 fluid>
                  <v-layout row>
                     <v-flex xs12>
                        <finance-cash-receive-new></finance-cash-receive-new>
                     </v-flex>
                  </v-layout>
               </v-container>
            </v-content>
...
mounted: function() {
    ...
    this.$store.commit("cashNew/set_object", ["cash_type_code", "CASH.RECEIVE"])
            ...
            if (acc_to_id != 0) {
               for (let acc of resy.records) {
                  if (acc.account_id == acc_to_id)
                     this.$store.commit("cashNew/set_object", ["selected_account_to", acc])
...
methods : {
   ...
   components: {
        ...
        "finance-cash-receive-new" : httpVueLoader("../components/finance-cash-receive-new-3.vue<?= $ts; ?>")
      }
    })
    </script>
    <style>
