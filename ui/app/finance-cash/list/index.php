         ...
         <v-app id="oneApp"  v-if="one_token">
            ...
            <!-- <common-notification></common-notification> -->
               <v-container pt-1 pl-1 pr-1 fluid>
                  <v-layout row>
                     <v-flex xs12>
                        <finance-cash-account-list v-show="selected_tab=='TAB.01'"></finance-cash-account-list>
                        <finance-cash-list v-show="selected_tab=='TAB.02'"></finance-cash-list>
                        <finance-cash-new-dialog></finance-cash-new-dialog>
...
computed : {
    ...
    tabs () {
      return this.$store.state.cash.tabs
    },

    selected_tab : {
      get () { return this.$store.state.cash.selected_tab },
      set (v) { this.$store.commit('cash/set_object', ['selected_tab', v]) }
    },

    tmp_acc_id : {
      get () { return this.$store.state.cash.tmp_acc_id },
      set (v) { this.$store.commit('cash/set_object', ['tmp_acc_id', v]) }
    },
...
methods : {
   ...
   components: {
        ...
        "finance-cash-list" : httpVueLoader("../components/finance-cash-list.vue<?php echo $ts ?>"),
        "finance-cash-account-list" : httpVueLoader("../components/finance-cash-account-list.vue<?php echo $ts ?>"),
        "finance-cash-new-dialog" : httpVueLoader("../components/finance-cash-new-dialog.vue<?= $ts; ?>"),
        "calculator" : httpVueLoader("../../common/components/common-dialog-calculator.vue<?= $ts; ?>")
      }
    })
    </script>
    <style>
