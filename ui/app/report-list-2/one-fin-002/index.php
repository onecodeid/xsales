...
<html lang="en">
   ...
   <head>
      ...
      <title>One :: Laporan Buku Besar</title>
...
beforeMount: function() {
   <?php
      if (isset($_GET['sdate']))
      {
         echo "this.".'$'."store.commit('fin002/set_object', ['s_date', '{$_GET['sdate']}'])\n";
      }

      if (isset($_GET['edate']))
      {
         echo "this.".'$'."store.commit('fin002/set_object', ['e_date', '{$_GET['edate']}'])\n";
      }
      ?>
},
...
mounted: function() {
    ...
    this.$store.dispatch("report_param/search_account").then((x) => {
      <?php
      if (isset($_GET['accid']))
      {
         echo "for(let a of x.records) { if(a.account_id=={$_GET['accid']}) this.".'$'."store.commit('fin002/set_object', ['selected_account', a]) }\n";
      }
      ?>
...
methods : {
   ...
   components: {
        ...
        "report-fin-002" : httpVueLoader("../components/report-data-fin-002-2.vue?t=<?=date('YmdHis')?>"),
