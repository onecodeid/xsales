            ...
            <v-layout row wrap>
                ...
                <v-flex xs5>
                    ...
                    <v-layout row wrap mt-1>
                        <v-flex xs12 class="text-xs-right" pl-2 offset-xs0>
                            <v-text-field
                                solo
                                hide-details
            ...
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    <tr class="cyan lighten-5">
                        ...
                        <td class="py-2 px-2 text-xs-left"><b>Saldo Awal</b></td>
                        ...
                        <td class="py-2 px-2 text-xs-right">
                            {{ one_money(props.item.details[0]?props.item.details[0].log_before_qty:0) }}  <span class="grey--text">{{ props.item.unit_name }}</span>
                        </td>
                    ...
                    <tr v-for="(d, i) in props.item.details" :key="'d-'+props.index+'-'+i">
                        <td class="py-1 px-2">{{ d.log_date }}</td>
                        <td class="py-1 px-2">{{ d.log_type_name }}</td>
                        <td class="py-1 px-2">{{ d.log_type_text }} <a href="#" @click="logDetailHref($event, d)" v-show="d.log_ref_number!=''">#{{ d.log_ref_number }}</a></td>
                        <td class="py-1 px-2">{{ d.warehouse_name }}</td>
                        <td class="py-1 px-2 text-xs-right">{{ one_money(d.log_qty) }} <span class="grey--text">{{ props.item.unit_name }}</span></td>
                        <td class="py-1 px-2 text-xs-right">{{ one_money(d.log_after_qty) }} <span class="grey--text">{{ props.item.unit_name }}</span></td>
                        <!-- <td>{{ d.log_type }}</td>
                        <td :colspan="headers.length">
                        </td> -->
                    </tr>
                    <tr>
                        <td class="py-1 px-2" colspan="3"></td>
                        <td class="py-1 px-2" colspan="2"><b>Saldo Akhir</b></td>
                        <td class="py-1 px-2 text-xs-right">{{ props.item.details.length > 0 ? props.item.details[props.item.details.length-1].log_after_qty : 0 }} <span class="grey--text">{{ props.item.unit_name }}</span></td>
                    </tr>
...
module.exports = {
    components : {
        ...
        "item-detail" : httpVueLoader("./report-data-iv-007-detail.vue")
    },
    ...
    computed : {
        items () {
            return this.$store.state.iv007.items
        },
        ...
        log_id () {
            return this.$store.state.iv007.selected_iv007.M_JournalID
        },

        query : {
            get () { return this.$store.state.iv007.search },
            set (v) { this.$store.commit('iv007/set_object', ['search', v]) }
        },

        curr_page : {
            get () { return this.$store.state.iv007.current_page },
            set (v) { this.$store.commit('iv007/set_object', ['current_page', v]) }
        },

        xtotal_page () {
            return this.$store.state.iv007.total_item_page
        },

        text_post () {
            let j = this.$store.state.iv007.selected_log
            return "Apakah anda yakin akan melakukan Posting Jurnal tersebut ?"
        },

        s_date : {
            get () { return this.$store.state.iv007.s_date },
            set (v) { this.$store.commit('iv007/set_common', ['s_date', v]) }
        },

        e_date : {
            get () { return this.$store.state.iv007.e_date },
            set (v) { this.$store.commit('iv007/set_common', ['e_date', v]) }
        },

        title() {
            return "LAPORAN RINCIAN PERSEDIAAN BARANG"
        },
        ...
        selected_warehouse : {
            get () { return this.$store.state.iv007.selected_warehouse },
            set (v) { this.$store.commit('iv007/set_object', ['selected_warehouse', v]) }
        },

        categories () {
            return this.$store.state.iv007.categories
        },

        selected_category : {
            get () { return this.$store.state.iv007.selected_category },
            set (v) { 
                this.$store.commit('iv007/set_object', ['selected_category', v]) 
                this.search()
            }
        }
    },
    ...
    methods : {
        ...
        confirm_del (x) {
            this.$store.dispatch('iv007/del', {id:x.data})
        },
        ...
        confirm_post (x) {
            this.$store.dispatch('iv007/post', {id:x.data})
        },

        select (x) {
            this.$store.commit('iv007/set_object', ['selected_log', x])
        },

        search () {
            return this.$store.dispatch('iv007/search', {})
        },

        change_page(x) {
            this.curr_page = x
            this.$store.dispatch('iv007/search', {})
        },

        change_s_date(x) {
            this.$store.commit('iv007/set_common', ['s_date', x.new_date])
            this.$store.dispatch('iv007/search')
        },

        change_e_date(x) {
            this.$store.commit('iv007/set_common', ['e_date', x.new_date])
            this.$store.dispatch('iv007/search')
        },
        ...
        print_log (x) {
            this.select(x)
            let so = x
            this.report_url = this.$store.state.iv007.URL+"report/one_sales_002?id="+so.log_id
        ...
        detail (x, y) {
            ...
            this.$store.commit('iv007/set_object', ['selected_item', y])
            this.$store.commit('iv007/set_object', ['dialog_detail', true])
            // let sc = y
            // this.$store.commit('log_new/set_common', ['edit', true])
            ...
            // let scv = this.$store.state.iv007.selected_log
        ...
        printMe () {
            this.$store.dispatch("iv007/collect").then((x) => {
                window.open(this.$store.state.iv007.URL + 'report/one_iv_007/excel?' + x)
            })
        },
        ...
        logDetailHref(e, d) {
            e.preventDefault()
            let url = ''
            if (d.log_type == 'SALES.DELIVERY') url = 'sales-delivery'
            if (d.log_type == 'PURCHASE.RECEIVE') url = 'purchase-receive'

            window.open('../../'+url+'/view/d'+d.log_ref_id)
        }
    },
    ...
    mounted () {
        ...
        this.$store.dispatch('iv007/search').then((x) => {
            this.$store.dispatch('iv007/search_category').then((x) => {
