        ...
        <v-card-text class="pt-2">
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    <tr class="lighten-5">
                        <td class="py-3 px-2 text-xs-left cyan lighten-5">
                        ...
                        <td class="py-3 px-2 text-xs-right"><b>{{ one_money(Math.round(props.item.log_a4_qty)) }}</b> <span class="grey--text caption">{{ props.item.unit_name }}</span></td>
                        <td class="py-3 px-2 text-xs-right"><span class="grey--text caption">Rp</span> <b>{{ one_money(props.item.item_hpp) }}</b></td>
                        ...
                        <td class="py-3 px-2 text-xs-right"><span class="grey--text caption">Rp</span> <b>{{ one_money(props.item.item_hpp * props.item.log_a4_qty) }}</b></td>
...
module.exports = {
    components : {
        ...
        "item-detail" : httpVueLoader("./report-data-iv-006-detail.vue")
    },
    ...
    computed : {
        items () {
            return this.$store.state.iv006.items
        },
        ...
        log_id () {
            return this.$store.state.iv006.selected_iv006.M_JournalID
        },

        query : {
            get () { return this.$store.state.iv006.search },
            set (v) { this.$store.commit('iv006/set_object', ['search', v]) }
        },

        curr_page : {
            get () { return this.$store.state.iv006.current_page },
            set (v) { this.$store.commit('iv006/set_object', ['current_page', v]) }
        },

        xtotal_page () {
            return this.$store.state.iv006.total_item_page
        },

        text_post () {
            let j = this.$store.state.iv006.selected_log
            return "Apakah anda yakin akan melakukan Posting Jurnal tersebut ?"
        },

        s_date : {
            get () { return this.$store.state.iv006.s_date },
            set (v) { this.$store.commit('iv006/set_common', ['s_date', v]) }
        },

        e_date : {
            get () { return this.$store.state.iv006.e_date },
            set (v) { this.$store.commit('iv006/set_common', ['e_date', v]) }
        },

        title() {
            return "LAPORAN RINGKASAN PERSEDIAAN BARANG"
        },
        ...
        selected_warehouse : {
            get () { return this.$store.state.iv006.selected_warehouse },
            set (v) { this.$store.commit('iv006/set_object', ['selected_warehouse', v]) }
        },
    ...
    methods : {
        ...
        confirm_del (x) {
            this.$store.dispatch('iv006/del', {id:x.data})
        },
        ...
        confirm_post (x) {
            this.$store.dispatch('iv006/post', {id:x.data})
        },

        select (x) {
            this.$store.commit('iv006/set_object', ['selected_log', x])
        },

        search () {
            return this.$store.dispatch('iv006/search', {})
        },

        change_page(x) {
            this.curr_page = x
            this.$store.dispatch('iv006/search', {})
        },

        change_s_date(x) {
            this.$store.commit('iv006/set_common', ['s_date', x.new_date])
            this.$store.dispatch('iv006/search')
        },

        change_e_date(x) {
            this.$store.commit('iv006/set_common', ['e_date', x.new_date])
            this.$store.dispatch('iv006/search')
        },
        ...
        print_log (x) {
            this.select(x)
            let so = x
            this.report_url = this.$store.state.iv006.URL+"report/one_sales_002?id="+so.log_id
        ...
        detail (x, y) {
            ...
            this.$store.commit('iv006/set_object', ['selected_item', y])
            this.$store.commit('iv006/set_object', ['dialog_detail', true])
            // let sc = y
            // this.$store.commit('log_new/set_common', ['edit', true])
            ...
            // let scv = this.$store.state.iv006.selected_log
        ...
        printMe () {
            this.$store.dispatch("iv006/collect").then((x) => {
                window.open(this.$store.state.iv006.URL + 'report/one_iv_006/excel?' + x)
            })
        }
    },
    ...
    mounted () {
        ...
        this.$store.dispatch('iv006/search').then((x) => {
