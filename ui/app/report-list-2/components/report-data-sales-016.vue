            ...
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    <tr class="cyan lighten-4">
                        <td class="py-2 px-2 text-xs-left">{{ props.item.category_name }}</td>
                        <td :colspan="2" class="py-2 px-2 text-xs-left">» <b>{{ props.item.item_name }}</b></td>
                        ...
                        <td>&nbsp;</td>
                        <td class="pa-2 text-xs-right"><span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(props.item.detail_subtotal)) }}</b></td>
                        <td class="pa-2 text-xs-right"><span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(props.item.detail_disctotal)) }}</b></td>
                    ...
                    <tr v-for="(inv, n) in props.item.invoices" :key="'I'+props.item.item_id+'V'+inv.invoice_id">
                        ...
                        <td class="text-xs-left pa-2" @click="select(props.item)" v-show="!is_sales">
                            ...
                            <i class="caption cyan--text">{{ inv.sales_name }}</i>
                        </td>
                        ...
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(inv.detail_price)) }}</b>
                        </td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(inv.detail_subtotal)) }}</b>
                        </td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(inv.detail_disctotal)) }}</b>
                        </td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(inv.detail_ppnamount)) }}</b>
                        </td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
...
module.exports = {
    ...
    computed : {
        invoices () {
            return this.$store.state.sales016.invoices
        },
        ...
        invoice_id () {
            return this.$store.state.sales016.selected_sales016.M_JournalID
        },

        query : {
            get () { return this.$store.state.sales016.search },
            set (v) { this.$store.commit('sales016/update_search', v) }
        },

        query : {
            get () { return this.$store.state.sales016.search },
            set (v) { this.$store.commit('sales016/set_object', ['search', v]) }
        },

        curr_page : {
            get () { return this.$store.state.sales016.current_page },
            set (v) { this.$store.commit('sales016/set_object', ['current_page', v]) }
        },

        xtotal_page () {
            return this.$store.state.sales016.total_invoice_page
        },

        text_post () {
            let j = this.$store.state.sales016.selected_invoice
            return "Apakah anda yakin akan melakukan Posting Jurnal tersebut ?"
        },

        s_date : {
            get () { return this.$store.state.sales016.s_date },
            set (v) { this.$store.commit('sales016/set_common', ['s_date', v]) }
        },

        e_date : {
            get () { return this.$store.state.sales016.e_date },
            set (v) { this.$store.commit('sales016/set_common', ['e_date', v]) }
        },

        title() {
            return "DAFTAR PENJUALAN CUSTOMER BARU"
        },
        ...
        categories () {
            return this.$store.state.sales016.categories
        },

        selected_category : {
            get () { return this.$store.state.sales016.selected_category },
            set (v) { 
                this.$store.commit('sales016/set_object', ['selected_category', v]) 
                this.search()
            }
        },
        ...
        selected_staff : {
            get () { return this.$store.state.sales016.selected_staff },
            set (v) { 
                this.$store.commit('sales016/set_object', ['selected_staff', v]) 
                this.search()
            }
        }
    },
    ...
    methods : {
        ...
        confirm_del (x) {
            this.$store.dispatch('sales016/del', {id:x.data})
        },
        ...
        confirm_post (x) {
            this.$store.dispatch('sales016/post', {id:x.data})
        },

        select (x) {
            this.$store.commit('sales016/set_object', ['selected_invoice', x])
        },

        search () {
            return this.$store.dispatch('sales016/search', {})
        },

        change_page(x) {
            this.curr_page = x
            this.$store.dispatch('sales016/search', {})
        },

        change_s_date(x) {
            this.$store.commit('sales016/set_common', ['s_date', x.new_date])
            this.$store.dispatch('sales016/search')
        },

        change_e_date(x) {
            this.$store.commit('sales016/set_common', ['e_date', x.new_date])
            this.$store.dispatch('sales016/search')
        },
        ...
        print_invoice (x) {
            this.select(x)
            let so = x
            this.report_url = this.$store.state.sales016.URL+"report/one_sales_002?id="+so.invoice_id
        ...
        detail (x, y) {
            ...
            let scv = this.$store.state.sales016.selected_invoice
        ...
        printMe () {
            this.$store.dispatch("sales016/collect").then((x) => {
                window.open(this.$store.state.sales016.URL + 'report/one_sales_016/excel?' + x)
            })
        }
    },
    ...
    mounted () {
        ...
        this.$store.dispatch('sales016/search', {})
        this.$store.dispatch('sales016/search_category')
