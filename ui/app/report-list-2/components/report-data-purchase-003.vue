            ...
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    <tr class="cyan lighten-4">
                        <td>&nbsp;</td>
                        <td :colspan="1" class="py-2 px-3 text-xs-left"><b>{{ props.item.vendor_name }}</b></td>
                    ...
                    <tr v-for="(inv, n) in props.item.bills" :key="'B'+inv.bill_id">
                       
                        <td class="text-xs-left pa-2" @click="select(props.item)">{{ inv.bill_date }}</td>
                        ...
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(inv.bill_subtotal)) }}</b>
                        </td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(inv.bill_disctotal)) }}</b>
                        </td>
...
module.exports = {
    ...
    computed : {
        bills () {
            return this.$store.state.purchase003.bills
        },
        ...
        bill_id () {
            return this.$store.state.purchase003.selected_purchase003.M_JournalID
        },

        query : {
            get () { return this.$store.state.purchase003.search },
            set (v) { this.$store.commit('purchase003/set_object', ['search', v]) }
        },

        curr_page : {
            get () { return this.$store.state.purchase003.current_page },
            set (v) { this.$store.commit('purchase003/set_object', ['current_page', v]) }
        },

        xtotal_page () {
            return this.$store.state.purchase003.total_bill_page
        },

        text_post () {
            let j = this.$store.state.purchase003.selected_bill
            return "Apakah anda yakin akan melakukan Posting Jurnal tersebut ?"
        },

        s_date : {
            get () { return this.$store.state.purchase003.s_date },
            set (v) { this.$store.commit('purchase003/set_common', ['s_date', v]) }
        },

        e_date : {
            get () { return this.$store.state.purchase003.e_date },
            set (v) { this.$store.commit('purchase003/set_common', ['e_date', v]) }
        },

        title() {
            return "DAFTAR PEMBELIAN PER VENDOR"
        },
    ...
    methods : {
        ...
        confirm_del (x) {
            this.$store.dispatch('purchase003/del', {id:x.data})
        },
        ...
        confirm_post (x) {
            this.$store.dispatch('purchase003/post', {id:x.data})
        },

        select (x) {
            this.$store.commit('purchase003/set_object', ['selected_bill', x])
        },

        search () {
            return this.$store.dispatch('purchase003/search', {})
        },

        change_page(x) {
            this.curr_page = x
            this.$store.dispatch('purchase003/search', {})
        },

        change_s_date(x) {
            this.$store.commit('purchase003/set_common', ['s_date', x.new_date])
            this.$store.dispatch('purchase003/search')
        },

        change_e_date(x) {
            this.$store.commit('purchase003/set_common', ['e_date', x.new_date])
            this.$store.dispatch('purchase003/search')
        },
        ...
        print_bill (x) {
            this.select(x)
            let so = x
            this.report_url = this.$store.state.purchase003.URL+"report/one_sales_002?id="+so.bill_id
        ...
        detail (x, y) {
            ...
            let scv = this.$store.state.purchase003.selected_bill
        ...
        printMe () {
            this.$store.dispatch("purchase003/collect").then((x) => {
                window.open(this.$store.state.purchase003.URL + 'report/one_purchase_003/excel?' + x)
            })
        }
    },
