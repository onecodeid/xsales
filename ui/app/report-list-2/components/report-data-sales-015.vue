            ...
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    <tr class="cyan lighten-4">
                        <td>&nbsp;</td>
                        <td :colspan="1" class="py-2 px-2 text-xs-left"><b>{{ props.item.customer_name }}</b></td>
                        ...
                        <td class="pa-2 text-xs-right"><span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(props.item.invoice_dp)) }}</b></td>
                    ...
                    <tr v-for="(inv, n) in props.item.invoices" :key="'inv-'+inv.invoice_id">
                        <!-- <td class="text-xs-left pa-1" @click="select(props.item)" style="writing-mode:tb-rl;height:auto;transform:scale(-1)">
                            <span class="orange white--text px-2 py-3 d-block" v-if="inv.invoice_unpaid==0">LUNAS</span>
                            <span class="success pa-2 py-3 d-block white--text" v-if="inv.invoice_unpaid>0&&inv.invoice_paid>0">S</span>
                            <span class="grey white--text pa-2 py-3 d-block" v-if="inv.invoice_paid==0">BARU</span>
                        </td> -->
                        <td class="text-xs-left pa-2" @click="select(props.item)">{{ inv.invoice_date }}</td>
                        <td class="text-xs-left pa-2" @click="select(props.item)"><a href="#" @click="select(props.item),detail($event, inv)">{{ inv.invoice_number }}</a></td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(inv.invoice_subtotal)) }}</b>
                        </td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(inv.invoice_disctotal)) }}</b>
                        </td>

                        <td class="text-xs-right pa-2" @click="select(props.item)">
                            <div v-show="inv.invoice_ppnvalue>0"><span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(inv.invoice_ppn)) }}</b></div>
                            <div v-show="inv.invoice_ppnvalue==0"><span class="red--text">Tanpa PPN</span></div>
                        </td>
                        

                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(inv.invoice_shipping)) }}</b>
                        </td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(inv.invoice_dp)) }}</b>
                        </td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(inv.invoice_grand_total)) }}</b>
                        </td>
                        <td class="text-xs-left pa-1" @click="select(props.item)">
                            <v-btn color="orange white--text pa-1 d-block text-xs-center ma-0" v-if="inv.invoice_unpaid==0" block small @click="searchHistory(inv)">LUNAS</v-btn>
                        ...
                        <td class="text-xs-center pa-0" @click="select(props.item)">
                            <!-- <div class="row">
                                <div class="col-12">
                                    <v-btn color="orange" class="btn-icon ma-0" small @click="print_invoice(inv)" dark title="Cetak invoice"><v-icon>print</v-icon></v-btn>
...
module.exports = {
    components : {
        ...
        "history" : httpVueLoader("./report-data-sales-015-payment-history.vue")
    },
    ...
    computed : {
        invoices () {
            return this.$store.state.sales015.invoices
        },
        ...
        invoice_id () {
            return this.$store.state.sales015.selected_sales015.M_JournalID
        },

        query : {
            get () { return this.$store.state.sales015.search },
            set (v) { this.$store.commit('sales015/update_search', v) }
        },

        query : {
            get () { return this.$store.state.sales015.search },
            set (v) { this.$store.commit('sales015/set_object', ['search', v]) }
        },

        curr_page : {
            get () { return this.$store.state.sales015.current_page },
            set (v) { this.$store.commit('sales015/set_object', ['current_page', v]) }
        },

        xtotal_page () {
            return this.$store.state.sales015.total_invoice_page
        },

        text_post () {
            let j = this.$store.state.sales015.selected_invoice
            return "Apakah anda yakin akan melakukan Posting Jurnal tersebut ?"
        },

        s_date : {
            get () { return this.$store.state.sales015.s_date },
            set (v) { this.$store.commit('sales015/set_common', ['s_date', v]) }
        },

        e_date : {
            get () { return this.$store.state.sales015.e_date },
            set (v) { this.$store.commit('sales015/set_common', ['e_date', v]) }
        },

        title() {
            return "DAFTAR PENJUALAN PELANGGAN"
        },
    ...
    methods : {
        ...
        confirm_del (x) {
            this.$store.dispatch('sales015/del', {id:x.data})
        },
        ...
        confirm_post (x) {
            this.$store.dispatch('sales015/post', {id:x.data})
        },

        select (x) {
            this.$store.commit('sales015/set_object', ['selected_invoice', x])
        },

        search () {
            return this.$store.dispatch('sales015/search', {})
        },

        change_page(x) {
            this.curr_page = x
            this.$store.dispatch('sales015/search', {})
        },

        change_s_date(x) {
            this.$store.commit('sales015/set_common', ['s_date', x.new_date])
            this.$store.dispatch('sales015/search')
        },

        change_e_date(x) {
            this.$store.commit('sales015/set_common', ['e_date', x.new_date])
            this.$store.dispatch('sales015/search')
        },
        ...
        print_invoice (x) {
            this.select(x)
            let so = x
            this.report_url = this.$store.state.sales015.URL+"report/one_sales_002?id="+so.invoice_id
        ...
        detail (x, y) {
            ...
            let scv = this.$store.state.sales015.selected_invoice
        ...
        printMe () {
            this.$store.dispatch("sales015/collect").then((x) => {
                window.open(this.$store.state.sales015.URL + 'report/one_sales_015/excel?' + x)
            })
        },

        searchHistory (x) {
            this.select(x)
            this.$store.commit('sales015/set_object', ['dialog_history', true])
            return this.$store.dispatch('sales015/searchHistory')
        }
    },
