            ...
            <v-card-text>
                ...
                <v-data-table 
                    ...
                    <template slot="items" slot-scope="props">
                        <tr class="">
                            <td class="py-3 px-2 text-xs-left">{{ props.item.invoice_date }}<br>{{ props.item.invoice_number }}</td>
                            <td class="py-2 px-2 text-xs-left">{{ props.item.customer_name }}</td>
...
module.exports = {
    ...
    computed : {
        dialog : {
            get () { return this.$store.state.sales017.dialog_detail },
            set (v) { 
                this.$store.commit('sales017/set_object', ['dialog_detail', v])
            }
        },

        selected_item () {
            if (this.$store.state.sales017.selected_item)
                return this.$store.state.sales017.selected_item
            return {}
        },

        invoices () {
            return this.selected_item.invoices ? this.selected_item.invoices : []
        },
        ...
        details () {
            return {
                ...
                'PERIODE' : this.$store.state.sales017.s_date + ' s/d ' + this.$store.state.sales017.e_date
