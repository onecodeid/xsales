            ...
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    ...
                    <tr class="">
                        ...
                        <td class="py-2 px-2 text-xs-left">{{ props.item.log_ref }}</td>
                        <td class="py-2 px-2 text-xs-left">{{ props.item.log_item_code }}</td>
                        <td class="py-2 px-2 text-xs-left">{{ props.item.log_item }}</td>
                        <td class="py-2 px-2 text-xs-left">{{ props.item.log_desc }}</td>
                        <td class="py-2 px-2 text-xs-right blue--text" :class="{'font-weight-bold':props.item.log_in>0}">{{ oneMoney(props.item.log_in) }}</td>
                        <td class="py-2 px-2 text-xs-right red--text" :class="{'font-weight-bold':props.item.log_out>0}">{{ oneMoney(props.item.log_out) }}</td>
...
module.exports = {
    ...
    computed : {
        ...
        s_date : {
            get () { return this.$store.state.iv004.s_date },
            set (v) { this.$store.commit('iv004/set_common', ['s_date', v]) }
        },

        e_date : {
            get () { return this.$store.state.iv004.e_date },
            set (v) { this.$store.commit('iv004/set_common', ['e_date', v]) }
        },

        title() {
            return "LAPORAN MUTASI BARANG PER GUDANG"
        },

        query : {
            get () { return this.$store.state.iv004.search },
            set (v) { this.$store.commit('iv004/set_object', ['search', v]) }
        },

        items () {
            return this.$store.state.iv004.items
        },

        selected_item : {
            get () { return this.$store.state.iv004.selected_item },
            set (v) { this.$store.commit('iv004/set_object', ['selected_item', v]) }
        },
        ...
        selected_warehouse : {
            get () { return this.$store.state.iv004.selected_warehouse },
            set (v) { 
                this.$store.commit('iv004/set_object', ['selected_warehouse', v]) 
                this.search()
            }
        },
    ...
    methods : {
        ...
        change_s_date(x) {
            this.$store.commit('iv004/set_common', ['s_date', x.new_date])
            this.$store.dispatch('iv004/search')
        },

        change_e_date(x) {
            this.$store.commit('iv004/set_common', ['e_date', x.new_date])
            this.$store.dispatch('iv004/search')
        },

        search () {
            return this.$store.dispatch('iv004/search', {})
        },

        printMe () {
            this.$store.dispatch("iv004/collect").then((x) => {
                window.open(this.$store.state.iv004.URL + 'report/one_iv_004/excel?' + x)
            })
        },
        ...
        journal(x) {
            this.$store.commit('iv004/set_object', ['selected_journal', x])
            // this.select(x)
            ...
            this.$store.dispatch('iv004/searchJournalDetail').then((x) => {
                let accs = []
                for (let y of x)
                    accs.push(y.account)
    ...
    mounted () {
        ...
        // this.$store.dispatch("iv004/search").then(() => {
