            ...
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    <tr class="cyan lighten-4">
                        <td :colspan="4" class="py-2 px-2 text-xs-left"><b>{{ props.item.customer_name }}</b></td>
                    ...
                    <tr v-for="(inv, n) in props.item.invoices">
                        ...
                        <td class="text-xs-center pa-2" @click="select(props.item)">{{ inv.invoice_due_date }}</td>
                        ...
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> 
                                <b>{{ inv.invoice_diff_date > 0 && inv.invoice_diff_date <= 30 ? one_money(Math.round(inv.invoice_unpaid)) : 0 }}</b>
                        </td>

                        <td class="text-xs-right pa-2" @click="select(props.item)">
                            <span class="grey--text caption">Rp</span> 
                                <b>{{ inv.invoice_diff_date > 30 && inv.invoice_diff_date <= 60 ? one_money(Math.round(inv.invoice_unpaid)) : 0 }}</b>
                        </td>
                        

                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> 
                                <b>{{ inv.invoice_diff_date > 60 && inv.invoice_diff_date <= 90 ? one_money(Math.round(inv.invoice_unpaid)) : 0 }}</b>
                        </td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> 
                                <b>{{ inv.invoice_diff_date > 90 && inv.invoice_diff_date <= 120 ? one_money(Math.round(inv.invoice_unpaid)) : 0 }}</b>
                        </td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> 
                                <b>{{ inv.invoice_diff_date > 120 ? one_money(Math.round(inv.invoice_unpaid)) : 0 }}</b>
                        </td>
...
module.exports = {
    ...
    computed : {
        invoices () {
            return this.$store.state.fin006.invoices
        },
        ...
        invoice_id () {
            return this.$store.state.fin006.selected_fin006.M_JournalID
        },

        query : {
            get () { return this.$store.state.fin006.search },
            set (v) { this.$store.commit('fin006/update_search', v) }
        },

        query : {
            get () { return this.$store.state.fin006.search },
            set (v) { this.$store.commit('fin006/set_object', ['search', v]) }
        },

        curr_page : {
            get () { return this.$store.state.fin006.current_page },
            set (v) { this.$store.commit('fin006/set_object', ['current_page', v]) }
        },

        xtotal_page () {
            return this.$store.state.fin006.total_invoice_page
        },

        text_post () {
            let j = this.$store.state.fin006.selected_invoice
            return "Apakah anda yakin akan melakukan Posting Jurnal tersebut ?"
        },

        s_date : {
            get () { return this.$store.state.fin006.s_date },
            set (v) { this.$store.commit('fin006/set_common', ['s_date', v]) }
        },

        e_date : {
            get () { return this.$store.state.fin006.e_date },
            set (v) { this.$store.commit('fin006/set_common', ['e_date', v]) }
        },
    ...
    methods : {
        ...
        confirm_del (x) {
            this.$store.dispatch('fin006/del', {id:x.data})
        },
        ...
        confirm_post (x) {
            this.$store.dispatch('fin006/post', {id:x.data})
        },

        select (x) {
            this.$store.commit('fin006/set_object', ['selected_invoice', x])
        },

        search () {
            return this.$store.dispatch('fin006/search', {})
        },

        change_page(x) {
            this.curr_page = x
            this.$store.dispatch('fin006/search', {})
        },

        change_s_date(x) {
            this.$store.commit('fin006/set_common', ['s_date', x.new_date])
            this.$store.dispatch('fin006/search')
        },

        change_e_date(x) {
            this.$store.commit('fin006/set_common', ['e_date', x.new_date])
            this.$store.dispatch('fin006/search')
        },
        ...
        print_invoice (x) {
            this.select(x)
            let so = x
            this.report_url = this.$store.state.fin006.URL+"report/one_sales_002?id="+so.invoice_id
        ...
        detail (x, y) {
            ...
            let scv = this.$store.state.fin006.selected_invoice
        ...
        printMe () {
            this.$store.dispatch("fin006/collect").then((x) => {
                window.open(this.$store.state.fin006.URL + 'report/one_fin_006/excel?' + x)
            })
        }
    },
