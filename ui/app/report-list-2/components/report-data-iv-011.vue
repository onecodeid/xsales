                ...
                <v-flex xs12>
                    <template>
                        <v-tabs v-model="activeTab" class="mb-1">
                            <v-tab v-for="(w, n) in tabs" :key="'tab-'+n"
                                :class="'lighten-5 '+w.color.replace(/lighten/, '')+' grow'" :active-class="'cyan white--text'" >{{ w.title }} <span v-show="n<4">({{ items[n]?items[n].items.length:0 }})</span></v-tab>
                            <!-- <v-tab
                                class="cyan lighten-5 grow" active-class="cyan white--text" >ANALISIS</v-tab> -->
                        </v-tabs>
                    </template>
                </v-flex>
                <v-flex xs12 v-for="(itemx, n) in items" :key="n" v-show="activeTab==n">
                    <v-data-table 
                        :headers="headers"
                        :items="itemx.items"
                        :loading="false"
                        ...
                        <template slot="items" slot-scope="props">
                            <tr class="">
                                ...
                                <td class="py-2 px-2 text-xs-right">{{ oneMoney(props.item.omzet_freq) }}</td>
                                <td class="py-2 px-2 text-xs-right">{{ oneMoney(props.item.omzet_qty) }}</td>
                                <td class="py-2 px-2 text-xs-right">{{ oneMoney(props.item.item_stock) }}</td>
                        ...
                        <template slot="footer">
                            <tr class="cyan lighten-4 orange--text">
                                <td :colspan="3" class="py-2 px-3 text-xs-right orange lighten-3 white--text"><b>GRAND TOTAL</b></td>
                                <td :colspan="1" class="py-2 px-2 text-xs-right yellow lighten-3"><b>{{ oneMoney(grand_total[itemx.id][0]) }}</b></td>
                                <td :colspan="1" class="py-2 px-2 text-xs-right yellow lighten-3"><b>{{ oneMoney(grand_total[itemx.id][1]) }}</b></td>
                                <td :colspan="1" class="py-2 px-2 text-xs-right yellow lighten-3"><b>{{ oneMoney(grand_total[itemx.id][2]) }}</b></td>
                                <td :colspan="1" class="py-2 px-2 text-xs-right yellow lighten-3"></td>
                            </tr>
                        </template>
                    </v-data-table>
                </v-flex>
                        ...
                        <table class="v-datatable v-table theme--light">
                            <thead>
                                ...
                                <!-- <tr class="v-datatable__progress">
                                    <th colspan="7" class="column"></th></tr> -->
                            </thead>
...
module.exports = {
    ...
    computed : {
        ...
        activeTab : {
            get () { return this.$store.state.iv011.current_tab },
            set (v) { 
                this.$store.commit('iv011/set_object', ['current_tab', v]) 
            }
        },

        s_date : {
            get () { return this.$store.state.iv011.s_date },
            set (v) { this.$store.commit('iv011/set_common', ['s_date', v]) }
        },

        e_date : {
            get () { return this.$store.state.iv011.e_date },
            set (v) { this.$store.commit('iv011/set_common', ['e_date', v]) }
        },

        title() {
            return "KLASIFIKASI STOK"
        },

        query : {
            get () { return this.$store.state.iv011.search },
            set (v) { this.$store.commit('iv011/set_object', ['search', v]) }
        },

        items () {
            return this.$store.state.iv011.items
        },

        selected_item : {
            get () { return this.$store.state.iv011.selected_item },
            set (v) { this.$store.commit('iv011/set_object', ['selected_item', v]) }
        },
        ...
        selected_warehouse : {
            get () { return this.$store.state.iv011.selected_warehouse },
            set (v) { 
                this.$store.commit('iv011/set_object', ['selected_warehouse', v]) 
                this.search().then((x) => {
                    this.$store.dispatch("iv011/searchAnalytic")
                })
            }
        },
        ...
        analysis () {
            return this.$store.state.iv011.analysis
        }
    },
    ...
    methods : {
        ...
        change_s_date(x) {
            this.$store.commit('iv011/set_common', ['s_date', x.new_date])
            this.$store.dispatch('iv011/search')
        },

        change_e_date(x) {
            this.$store.commit('iv011/set_common', ['e_date', x.new_date])
            this.$store.dispatch('iv011/search')
        },

        search () {
            return this.$store.dispatch('iv011/search', {})
        },

        printMe () {
            this.$store.dispatch("iv011/collect").then((x) => {
                window.open(this.$store.state.iv011.URL + 'report/one_iv_009/excel?' + x)
            })
        },
        ...
        journal(x) {
            this.$store.commit('iv011/set_object', ['selected_journal', x])
            // this.select(x)
            ...
            this.$store.dispatch('iv011/searchJournalDetail').then((x) => {
                let accs = []
                for (let y of x)
                    accs.push(y.account)
        ...
        save () {
            this.$store.dispatch('iv011/saveAnalytic').then((x) => {
                this.snackbar = true
            })
        },
        ...
        changeAnalytic (code, desc) {
            ...
            this.$store.commit('iv011/set_object', ['analysis', ans])
        }
    },
    ...
    mounted () {
        ...
        // this.$store.dispatch("iv011/search").then(() => {
