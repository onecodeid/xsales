<template>
    <v-card>
                ...
                <v-flex xs2 pr-1>
                    ...
                    >
                        <template v-slot:append-outer>
                            <v-btn color="orange" :dark="!!selected_endyear" class="ma-0 ml-2 btn-icon" @click="printMe" :disabled="!selected_endyear">
                                <v-icon>print</v-icon>
                            </v-btn>  
                        </template>
        ...
        <v-card-text class="pt-2">
            <v-divider class="cyan mb-4"></v-divider>
            <!-- <h3 class="headline text-xs-center cyan white--text py-1"></h3> -->
            <!-- <h3 class="headline text-xs-center cyan white--text pt-2">{{ title }}</h3>
            <h5 class="text-xs-center subheading mb-5 cyan white--text pb-2"><b>Per {{ edate }}</b></h5> -->
            <v-layout row wrap class="subheading my-2">
                <v-flex xs3 offset-xs3>Modal per {{ sdate }}</v-flex>
                <v-flex xs3 class="text-xs-right"><span class="grey--text">Rp</span> {{ one_money(records.balance_start) }}</v-flex>
            </v-layout>
            <v-layout row wrap class="subheading my-2">
                <v-flex xs3 offset-xs3>Laba Bersih per {{ edate }}</v-flex>
                <v-flex xs3 class="text-xs-right"><span class="grey--text">Rp</span> {{ one_money(records.profit) }}</v-flex>
            </v-layout>
            <v-layout row wrap class="subheading my-2">
                <v-flex xs2 offset-xs7><v-divider></v-divider></v-flex>
            </v-layout>
            <v-layout row wrap class="subheading my-2 font-weight-bold">
                <v-flex xs3 offset-xs3>&nbsp;</v-flex>
                <v-flex xs3 class="text-xs-right"><span class="grey--text">Rp</span> {{ one_money(parseFloat(records.balance_start)+parseFloat(records.profit)) }}</v-flex>
            </v-layout>
            <v-layout row wrap class="subheading my-2">
                <v-flex xs3 offset-xs3>Prive</v-flex>
                <v-flex xs3 class="text-xs-right"><span class="grey--text">Rp</span> {{ one_money(records.prive) }}</v-flex>
            </v-layout>
            <v-layout row wrap class="subheading my-2">
                <v-flex xs2 offset-xs7><v-divider></v-divider></v-flex>
            </v-layout>
            <v-layout row wrap class="subheading my-2 font-weight-bold">
                <v-flex xs3 offset-xs3>Modal per {{ edate }}</v-flex>
                <!-- <v-flex xs3 class="text-xs-right"><span class="grey--text">Rp</span> {{ one_money(parseFloat(records.balance_start)+parseFloat(records.profit)-parseFloat(records.prive)) }}</v-flex> -->
                <v-flex xs3 class="text-xs-right"><span class="grey--text">Rp</span> {{ one_money(parseFloat(records.balance_start)+parseFloat(records.profit)-parseFloat(records.prive)) }}</v-flex>
            </v-layout>
        </v-card-text>
...
module.exports = {
    ...
    computed : {
        records () {
            return this.$store.state.fin009.records
        },
        ...
        // invoice_id () {
        //     return this.$store.state.fin009.selected_fin009.M_JournalID
        // },

        query : {
            get () { return this.$store.state.fin009.search },
            set (v) { this.$store.commit('fin009/update_search', v) }
        },

        // query : {
        //     get () { return this.$store.state.fin009.search },
        //     set (v) { this.$store.commit('fin009/set_object', ['search', v]) }
        // },

        // curr_page : {
        //     get () { return this.$store.state.fin009.current_page },
        //     set (v) { this.$store.commit('fin009/set_object', ['current_page', v]) }
        // },

        // xtotal_page () {
        //     return this.$store.state.fin009.total_invoice_page
        // },

        // text_post () {
        //     let j = this.$store.state.fin009.selected_invoice
        //     return "Apakah anda yakin akan melakukan Posting Jurnal tersebut ?"
        // },

        s_date : {
            get () { return this.$store.state.fin009.s_date },
            set (v) { this.$store.commit('fin009/set_common', ['s_date', v]) }
        },

        e_date : {
            get () { return this.$store.state.fin009.e_date },
            set (v) { this.$store.commit('fin009/set_common', ['e_date', v]) }
        },
    ...
    methods : {
        ...
        confirm_del (x) {
            this.$store.dispatch('fin009/del', {id:x.data})
        },
        ...
        confirm_post (x) {
            this.$store.dispatch('fin009/post', {id:x.data})
        },

        select (x) {
            this.$store.commit('fin009/set_object', ['selected_invoice', x])
        },

        search () {
            return this.$store.dispatch('fin009/search', {})
        },

        change_page(x) {
            this.curr_page = x
            this.$store.dispatch('fin009/search', {})
        },

        change_s_date(x) {
            this.$store.commit('fin009/set_common', ['s_date', x.new_date])
            this.$store.dispatch('fin009/search')
        },

        change_e_date(x) {
            this.$store.commit('fin009/set_common', ['e_date', x.new_date])
            this.$store.dispatch('fin009/search')
        },
        ...
        print_invoice (x) {
            this.select(x)
            let so = x
            this.report_url = this.$store.state.fin009.URL+"report/one_sales_002?id="+so.invoice_id
        ...
        detail (x, y) {
            ...
            let sc = y
            this.$store.commit('fin009/set_object', ['selected_account', y])
            this.$store.commit('fin009/set_object', ['dialog_detail', true])
            this.$store.commit('fin009/set_object', ['current_detail_page', 1])
            this.$store.dispatch('fin009/searchDetail')
            // this.$store.commit('invoice_new/set_common', ['edit', true])
            ...
            // let scv = this.$store.state.fin009.selected_invoice
        ...
        printMe () {
            this.$store.dispatch("fin009/collect").then((x) => {
                window.open(this.$store.state.fin009.URL + 'report/one_fin_009/excel?' + x)
            })
        }
    },
