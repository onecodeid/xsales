...
module.exports = {
    ...
    computed : {
        ...
        s_date : {
            get () { return this.$store.state.iv003.s_date },
            set (v) { this.$store.commit('iv003/set_common', ['s_date', v]) }
        },

        e_date : {
            get () { return this.$store.state.iv003.e_date },
            set (v) { this.$store.commit('iv003/set_common', ['e_date', v]) }
        },

        title() {
            return "LAPORAN KETERSEDIAAN BARANG PER GUDANG"
        },

        query : {
            get () { return this.$store.state.iv003.search },
            set (v) { this.$store.commit('iv003/set_object', ['search', v]) }
        },

        items () {
            return this.$store.state.iv003.items
        },

        selected_item : {
            get () { return this.$store.state.iv003.selected_item },
            set (v) { this.$store.commit('iv003/set_object', ['selected_item', v]) }
        },
        ...
        selected_warehouse : {
            get () { return this.$store.state.iv003.selected_warehouse },
            set (v) { 
                this.$store.commit('iv003/set_object', ['selected_warehouse', v]) 
                this.search()
            }
        },
    ...
    methods : {
        ...
        change_s_date(x) {
            this.$store.commit('iv003/set_common', ['s_date', x.new_date])
            this.$store.dispatch('iv003/search')
        },

        change_e_date(x) {
            this.$store.commit('iv003/set_common', ['e_date', x.new_date])
            this.$store.dispatch('iv003/search')
        },

        search () {
            return this.$store.dispatch('iv003/search', {})
        },

        printMe () {
            this.$store.dispatch("iv003/collect").then((x) => {
                window.open(this.$store.state.iv003.URL + 'report/one_iv_003/excel?' + x)
            })
        },
        ...
        journal(x) {
            this.$store.commit('iv003/set_object', ['selected_journal', x])
            // this.select(x)
            ...
            this.$store.dispatch('iv003/searchJournalDetail').then((x) => {
                let accs = []
                for (let y of x)
                    accs.push(y.account)
    ...
    mounted () {
        ...
        // this.$store.dispatch("iv003/search").then(() => {
