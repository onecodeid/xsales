        ...
        <v-card-text class="pt-2">
            <v-data-table 
                ...
                <template slot="items" slot-scope="props">
                    <tr class="lighten-5">
                        <td class="py-3 px-2 text-xs-left">
                            <a href="#" @click="detail($event, props.item)"><b>{{ props.item.item_code }}</b></a></td>
                        <td :colspan="2" class="py-3 px-2 text-xs-left">
                            <a href="#" @click="detail($event, props.item)"><b>{{ props.item.item_name }}</b></a></td>
                        <td class="py-3 px-2 text-xs-right"><b>{{ one_money(Math.round(props.item.stock_before_qty)) }}</b> <span class="grey--text caption">{{ props.item.unit_name }}</span></td>
                        <td class="py-3 px-2 text-xs-right"><b>{{ one_money(Math.round(props.item.stock_in_qty)) }}</b> <span class="grey--text caption">{{ props.item.unit_name }}</span></td>
                        ...
                        <td class="py-3 px-2 text-xs-right"><b>{{ one_money(Math.round(props.item.stock_after_qty)) }}</b> <span class="grey--text caption">{{ props.item.unit_name }}</span></td>
...
module.exports = {
    components : {
        ...
        "item-detail" : httpVueLoader("./report-data-iv-005-detail.vue")
    },
    ...
    computed : {
        items () {
            return this.$store.state.iv005.items
        },
        ...
        log_id () {
            return this.$store.state.iv005.selected_iv005.M_JournalID
        },

        query : {
            get () { return this.$store.state.iv005.search },
            set (v) { this.$store.commit('iv005/set_object', ['search', v]) }
        },

        curr_page : {
            get () { return this.$store.state.iv005.current_page },
            set (v) { this.$store.commit('iv005/set_object', ['current_page', v]) }
        },

        xtotal_page () {
            return this.$store.state.iv005.total_item_page
        },

        text_post () {
            let j = this.$store.state.iv005.selected_log
            return "Apakah anda yakin akan melakukan Posting Jurnal tersebut ?"
        },

        s_date : {
            get () { return this.$store.state.iv005.s_date },
            set (v) { this.$store.commit('iv005/set_common', ['s_date', v]) }
        },

        e_date : {
            get () { return this.$store.state.iv005.e_date },
            set (v) { this.$store.commit('iv005/set_common', ['e_date', v]) }
        },

        title() {
            return "LAPORAN PERGERAKAN PER GUDANG"
        },
        ...
        selected_warehouse : {
            get () { return this.$store.state.iv005.selected_warehouse },
            set (v) { this.$store.commit('iv005/set_object', ['selected_warehouse', v]) }
        }
    },
    ...
    methods : {
        ...
        confirm_del (x) {
            this.$store.dispatch('iv005/del', {id:x.data})
        },
        ...
        confirm_post (x) {
            this.$store.dispatch('iv005/post', {id:x.data})
        },

        select (x) {
            this.$store.commit('iv005/set_object', ['selected_log', x])
        },

        search () {
            return this.$store.dispatch('iv005/search', {})
        },

        change_page(x) {
            this.curr_page = x
            this.$store.dispatch('iv005/search', {})
        },

        change_s_date(x) {
            this.$store.commit('iv005/set_common', ['s_date', x.new_date])
            this.$store.dispatch('iv005/search')
        },

        change_e_date(x) {
            this.$store.commit('iv005/set_common', ['e_date', x.new_date])
            this.$store.dispatch('iv005/search')
        },
        ...
        print_log (x) {
            this.select(x)
            let so = x
            this.report_url = this.$store.state.iv005.URL+"report/one_sales_002?id="+so.log_id
        ...
        detail (x, y) {
            ...
            this.$store.commit('iv005/set_object', ['selected_item', y])
            this.$store.commit('iv005/set_object', ['dialog_detail', true])
            // let sc = y
            // this.$store.commit('log_new/set_common', ['edit', true])
            ...
            // let scv = this.$store.state.iv005.selected_log
        ...
        printMe () {
            this.$store.dispatch("iv005/collect").then((x) => {
                window.open(this.$store.state.iv005.URL + 'report/one_iv_005/excel?' + x)
            })
        }
    },
    ...
    mounted () {
        ...
        this.$store.dispatch('iv005/search').then((x) => {
