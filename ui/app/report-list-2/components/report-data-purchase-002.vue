        ...
        <v-card-text class="pt-2">
            <v-data-table 
                ...
                <template slot="headers" slot-scope="props">
                        ...
                        <tr>
                            <th v-for="(h, n) in headers" :key="'header-x-'+n" 
                        ...
                        <tr>
                            <th v-for="(h, n) in headers" :key="'header-y-'+n" 
                                role="columnheader" scope="col" :width="h.width" :aria-label="h.text+': Not sorted.'" aria-sort="none" :class="['column', 'text-xs-'+h.align, h.class]"
                ...
                <template slot="items" slot-scope="props">
                    <tr class="cyan lighten-4">
                        <td :colspan="4" class="py-2 px-2 text-xs-left"><b>{{ props.item.vendor_name }}</b></td>
                    ...
                    <tr v-for="(inv, n) in props.item.bills" :key="'bill-'+inv.bill_id">
                        <td class="text-xs-left pa-2" @click="select(props.item)"><a href="#" @click="select(props.item),detail($event, inv)">{{ inv.bill_number }}</a></td>
                        <td class="text-xs-center pa-2" @click="select(props.item)">{{ inv.bill_date }}</td>
                        <td class="text-xs-center pa-2" @click="select(props.item)">{{ inv.bill_due_date }}</td>
                        <td class="text-xs-center pa-2" @click="select(props.item)">{{ inv.term_duration }}</td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> 
                                <b>{{ one_money(Math.round(inv.bill_unpaid)) }}</b>
                        </td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> 
                                <b>{{ inv.bill_diff_date <= 0 ? one_money(Math.round(inv.bill_unpaid)) : 0 }}</b>
                        </td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> 
                                <b>{{ inv.bill_diff_date > 0 && inv.bill_diff_date <= 30 ? one_money(Math.round(inv.bill_unpaid)) : 0 }}</b>
                        </td>

                        <td class="text-xs-right pa-2" @click="select(props.item)">
                            <span class="grey--text caption">Rp</span> 
                                <b>{{ inv.bill_diff_date > 30 && inv.bill_diff_date <= 60 ? one_money(Math.round(inv.bill_unpaid)) : 0 }}</b>
                        </td>
                        

                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> 
                                <b>{{ inv.bill_diff_date > 60 && inv.bill_diff_date <= 90 ? one_money(Math.round(inv.bill_unpaid)) : 0 }}</b>
                        </td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> 
                                <b>{{ inv.bill_diff_date > 90 && inv.bill_diff_date <= 120 ? one_money(Math.round(inv.bill_unpaid)) : 0 }}</b>
                        </td>
                        <td class="text-xs-right pa-2" @click="select(props.item)" v-show="!is_sales">
                            <span class="grey--text caption">Rp</span> 
                                <b>{{ inv.bill_diff_date > 120 ? one_money(Math.round(inv.bill_unpaid)) : 0 }}</b>
                        </td>
                ...
                <template slot="footer" slot-scope="props">
                    <tr class="cyan lighten-4">
                        <td :colspan="4" class="py-2 px-2 text-xs-center"><b>GRAND TOTAL</b></td>
                        <td class="pa-2 text-xs-right"><span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(grand_total.total_unpaid)) }}</b></td>
                        <td class="pa-2 text-xs-right"><span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(grand_total.total_ongoing)) }}</b></td>
                        <td class="pa-2 text-xs-right"><span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(grand_total.total_30)) }}</b></td>
                        <td class="pa-2 text-xs-right"><span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(grand_total.total_60)) }}</b></td>
                        <td class="pa-2 text-xs-right"><span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(grand_total.total_90)) }}</b></td>
                        <td class="pa-2 text-xs-right"><span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(grand_total.total_120)) }}</b></td>
                        <td class="pa-2 text-xs-right"><span class="grey--text caption">Rp</span> <b>{{ one_money(Math.round(grand_total.total_rest)) }}</b></td>
                    </tr>
                </template>
            </v-data-table>
            <v-divider></v-divider>
...
module.exports = {
    components : {
        ...
        "bill-detail" : httpVueLoader("../../finance-bill/components/finance-bill-new.vue")
    },
    ...
    computed : {
        bills () {
            return this.$store.state.purchase002.bills
        },
        ...
        bill_id () {
            return this.$store.state.purchase002.selected_purchase002.M_JournalID
        },

        query : {
            get () { return this.$store.state.purchase002.search },
            set (v) { this.$store.commit('purchase002/update_search', v) }
        },

        query : {
            get () { return this.$store.state.purchase002.search },
            set (v) { this.$store.commit('purchase002/set_object', ['search', v]) }
        },

        curr_page : {
            get () { return this.$store.state.purchase002.current_page },
            set (v) { this.$store.commit('purchase002/set_object', ['current_page', v]) }
        },

        xtotal_page () {
            return this.$store.state.purchase002.total_bill_page
        },

        text_post () {
            let j = this.$store.state.purchase002.selected_bill
            return "Apakah anda yakin akan melakukan Posting Jurnal tersebut ?"
        },

        s_date : {
            get () { return this.$store.state.purchase002.s_date },
            set (v) { this.$store.commit('purchase002/set_common', ['s_date', v]) }
        },

        e_date : {
            get () { return this.$store.state.purchase002.e_date },
            set (v) { this.$store.commit('purchase002/set_common', ['e_date', v]) }
        },

        title() {
            return "LAPORAN USIA HUTANG"
        },
        ...
        grand_total () {
            return this.$store.state.purchase002.grand_total
        }
    },
    ...
    methods : {
        ...
        confirm_del (x) {
            this.$store.dispatch('purchase002/del', {id:x.data})
        },
        ...
        confirm_post (x) {
            this.$store.dispatch('purchase002/post', {id:x.data})
        },

        select (x) {
            this.$store.commit('purchase002/set_object', ['selected_bill', x])
        },

        search () {
            return this.$store.dispatch('purchase002/search', {})
        },

        change_page(x) {
            this.curr_page = x
            this.$store.dispatch('purchase002/search', {})
        },

        change_s_date(x) {
            this.$store.commit('purchase002/set_common', ['s_date', x.new_date])
            this.$store.dispatch('purchase002/search')
        },

        change_e_date(x) {
            this.$store.commit('purchase002/set_common', ['e_date', x.new_date])
            this.$store.dispatch('purchase002/search')
        },
        ...
        print_bill (x) {
            this.select(x)
            let so = x
            this.report_url = this.$store.state.purchase002.URL+"report/one_sales_002?id="+so.bill_id
        ...
        detail (x, y) {
            ...
            let scv = this.$store.state.purchase002.selected_bill
            ...
            for (let v of this.$store.state.bill_new.terms)
                if (v.term_id == sc.bill_term)
                    // this.$store.commit('bill_new/set_selected_term', v)
                    this.$store.commit('bill_new/set_object', ['selected_term',v])
        ...
        printMe () {
            this.$store.dispatch("purchase002/collect").then((x) => {
                window.open(this.$store.state.purchase002.URL + 'report/one_purchase_002/excel?' + x)
            })
        }
    },
