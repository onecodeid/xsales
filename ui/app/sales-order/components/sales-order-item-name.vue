...
module.exports = {
    ...
    computed : {
        dialog : {
            get () { return this.$store.state.sales_new.dialog_itemname },
            set (v) { this.$store.commit('sales_new/set_common', ['dialog_itemname', v]) }
        },

        item () {
            return this.$store.state.sales_new.selected_item.item
        },
        ...
        custom_name : {
            get () { return this.$store.state.sales_new.custom_item_name },
            set (v) { this.$store.commit('sales_new/set_common', ['custom_item_name', v]) }
        }
    },
    ...
    methods : {
        save () {
            if (this.$store.state.sales_new.edit) {
                this.$store.dispatch('sales_new/save_item_name').then(res => {
                    this.save_form()

                    this.$store.commit('sales/set_common', ['snackbar', true])
                    this.$store.commit('sales/set_common', ['snackbar_text', 'Nama Item berhasil diubah'])
                })
            } else {
                this.save_form()
            }
        ...
        save_form () {
            let items = JSON.parse(JSON.stringify(this.$store.state.sales_new.details))
            for (let i in items) {
            ...
            this.$store.commit('sales_new/set_details', items)
            this.dialog = false
        },
