...
module.exports = {
    ...
    computed : {
        saless () {
            return this.$store.state.offer.saless
        },
        ...
        sales_id () {
            return this.$store.state.offer.selected_sales.M_JournalID
        },

        query : {
            get () { return this.$store.state.offer.search },
            set (v) { this.$store.commit('offer/update_search', v) }
        },

        curr_page : {
            get () { return this.$store.state.offer.current_page },
            set (v) { this.$store.commit('offer/update_current_page', v) }
        },

        xtotal_page () {
            return this.$store.state.offer.total_sales_page
        },

        text_post () {
            let j = this.$store.state.offer.selected_sales
            return "Apakah anda yakin akan melakukan Posting Jurnal tersebut ?"
        },

        s_date : {
            get () { return this.$store.state.offer.s_date },
            set (v) { this.$store.commit('offer/set_common', ['s_date', v]) }
        },

        e_date : {
            get () { return this.$store.state.offer.e_date },
            set (v) { this.$store.commit('offer/set_common', ['e_date', v]) }
        },
        ...
        selected_leadtype : {
            get () { return this.$store.state.offer.selected_leadtype },
            set (v) { 
                this.$store.commit('offer/set_selected_leadtype', v) 
                this.search()
            }
        },
        ...
        selected_staff : {
            get () { return this.$store.state.offer.selected_staff },
            set (v) { 
                this.$store.commit('offer/set_selected_staff', v) 
                this.search()
            }
        }
    },
    ...
    methods : {
        ...
        add () {
            let x = []
            x.push(JSON.parse(JSON.stringify(this.$store.state.offer_new.detail_default)))
            ...
            this.$store.commit('offer_new/set_common', ['sales_stock', ''])
            this.$store.commit('offer_new/set_common', ['sales_delivery', ''])
            this.$store.commit('offer_new/set_common', ['sales_validity', ''])
            this.$store.commit('offer_new/set_common', ['sales_used', 'N'])
            this.$store.commit('offer_new/set_common', ['sales_shipping', 0])
            
            this.$store.commit('offer_new/set_selected_staff', null)
            this.$store.commit('offer_new/set_selected_customer', null)
            this.$store.commit('offer_new/set_selected_leadtype', null)
            this.$store.commit('offer_new/set_selected_paymentplan', null)
            this.$store.commit('offer_new/set_selected_term', null)
            this.$store.commit('offer_new/set_details', x)
            this.$store.commit('offer_new/set_common', ['dialog_new', true])
        },

        edit (x) {
            this.select(x)
            let sc = x
            this.$store.commit('offer_new/set_common', ['edit', true])
            ...
            this.$store.commit('offer_new/set_common', ['sales_stock', sc.sales_stocknote])
            this.$store.commit('offer_new/set_common', ['sales_delivery', sc.sales_delivery])
            this.$store.commit('offer_new/set_common', ['sales_validity', sc.sales_validity])
            this.$store.commit('offer_new/set_common', ['sales_used', sc.sales_used])
            this.$store.commit('offer_new/set_common', ['sales_shipping', sc.sales_shipping])

            this.$store.commit('offer_new/set_selected_customer', null)
            for (let v of this.$store.state.offer_new.customers)
                if (v.customer_id == sc.customer_id)
                    this.$store.commit('offer_new/set_selected_customer', v)

            this.$store.commit('offer_new/set_selected_leadtype', null)
            for (let v of this.$store.state.offer_new.leadtypes)
                if (v.leadtype_id == sc.sales_lead)
                    this.$store.commit('offer_new/set_selected_leadtype', v)

            this.$store.commit('offer_new/set_selected_staff', null)
            for (let v of this.$store.state.offer_new.staffs)
                if (sc.sales_staff == v.staff_id)
                    this.$store.commit('offer_new/set_selected_staff', v)

            this.$store.commit('offer_new/set_selected_paymentplan', null)
            for (let v of this.$store.state.offer_new.paymentplans)
                if (v.paymentplan_id == sc.sales_payment)
                    this.$store.commit('offer_new/set_selected_paymentplan', v)

            this.$store.commit('offer_new/set_selected_term', null)
            for (let v of this.$store.state.offer_new.terms)
                if (v.term_id == sc.sales_term)
                    this.$store.commit('offer_new/set_selected_term', v)

            let details = sc.details
            // let acc = this.$store.state.offer_new.accounts
            // for (let d of details)
            //     for (let a of acc)
            //         if (a.account_id == d.account)
            //             d.account = a
            console.log(details)
            this.$store.commit('offer_new/set_details', details)
        ...
        confirm_del (x) {
            this.$store.dispatch('offer/del', {id:x.data})
        },
        ...
        confirm_post (x) {
            this.$store.dispatch('offer/post', {id:x.data})
        },

        select (x) {
            this.$store.commit('offer/set_selected_sales', x)
        },

        search () {
            return this.$store.dispatch('offer/search', {})
        },
        ...
        change_s_date(x) {
            this.$store.commit('offer/set_common', ['s_date', x.new_date])
            this.$store.dispatch('offer/search')
        },

        change_e_date(x) {
            this.$store.commit('offer/set_common', ['e_date', x.new_date])
            this.$store.dispatch('offer/search')
        },

        printMe (x) {
            this.select(x)
            this.report_url = this.$store.state.offer.URL+"report/one_sales_004?id="+x.sales_id+
                "&token="+this.$store.state.offer.one_token
        ...
        convert (x) {
            ...
            this.$store.commit('sales_new/set_common', ['sales_id', 0])
            ...
            this.$store.commit('sales_new/set_selected_customer', {customer_id:sc.customer_id})
            ...
            this.$store.commit('sales_new/set_selected_offer', 
                {sales_id:x.sales_id,sales_number:x.sales_number,sales_date:x.sales_date})
            ...
            // this.$store.dispatch('offer/convert_to_sales')
        }
    },
    ...
    mounted () {
        ...
        this.$store.dispatch('offer_new/search_leadtype')
    }
}
</script>