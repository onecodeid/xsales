<template>
    <v-card>
        <v-card-title primary-title class="py-2 px-3 orange white--text">
            <v-layout row wrap>
                <v-flex xs8><h3 class="subheading text-xs-center">BIAYA - BIAYA</h3></v-flex>
                <v-flex xs4>
        ...
        <v-card-text>
            ...
            <v-layout row wrap>
                <v-flex xs12>
                    <v-btn color="primary btn-icon" @click="add_cost" :disabled="false"><v-icon>add</v-icon></v-btn>
                </v-flex>
            </v-layout>
        </v-card-text>
    </v-card>
...
module.exports = {
    ...
    computed : {
        accounts () { 
            return this.$store.state.item_new.assembly_accounts
        },
        ...
        view () {
            return this.$store.state.item_new.view
        },
    ...
    methods : {
        ...
        // select (x) {
        //     this.$store.commit('item_new/set_selected_item', x)
        // },

        add_cost () {
            let d = this.costs
            let dfl = JSON.parse(JSON.stringify(this.$store.state.item_new.assembly_cost_default))
            d.push(dfl)
        ...
        set_costs (d) {
            this.$store.commit('item_new/set_object', ['assembly_costs_tmp', d?d:this.costs])
        },
    ...
    mounted () {
        this.$store.dispatch('item_new/search_account')
    }
}
</script>