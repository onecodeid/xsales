...
class One_purchase_006 extends RPT_Controller
{
    var $report_code;
    var $wQty;
    ...
    function search()
    {
        ...
        $r = $this->r_reportpurchase->purchase_006($prm);
        $this->sys_ok($r);
    }
    ...
    function excel()
    {
        ...
        $filename = "laporan_penyelesaian_pembelian_".date('d.m.Y', strtotime($this->input->get('sdate')))."_".date('d.m.Y', strtotime($this->input->get('edate'))).".xls";
        ...
        if ($r)
        {
            ...
            $objPHPExcel = $objReader->load("./assets/template_purchase_006.xls");
            ...
            foreach ($r['records'] as $k => $v)
            {
                $mapData = ['A' => $no,
                            ...
                            'C' => $v['purchase_number'],
                            'D' => $v['vendor_name'],
                            'E' => $v['purchase_grandtotal'],
                            'F' => $v['receive_date'],
                            'G' => $v['receive_number'],
                            'H' => $v['purchase_done'] == 'Y' ? 'Diterima' : ($v['purchase_done'] == 'N' ? 'Belum Diterima' : 'Sebagian'),
                            'I' => $v['bill_date'],
                            'J' => $v['bill_number'],
                            'K' => $v['bill_lunas'] == 'Y' ? 'Lunas' : ($v['bill_lunas'] == 'X' ? 'Sebagian' : 'Belum'),
                            'L' => $v['sales_name'],
                            // 'M' => $w->detail_total,
                            // 'N' => $w->retur_qty,
                            // 'O' => $w->retur_nominal,
            ...
            $this->copyRowFull($as, $as, 6, $myLine);
            $as->setCellValue('E'.$myLine, "{$total}");
            ...
            $as->setCellvalue("M3", "=DATE(".date('Y,m,d', strtotime($this->sys_input['edate'])).")");
            // 
            // $baseRow = 'A5';
            // 
            ...
            // $as->setCellValue('A2', 'Periode : ' . date('d/m/Y', strtotime($this->input->get('sdate'))) . ' - ' .  date('d/m/Y', strtotime($this->input->get('edate'))));
                ...
                // $as->setCellValue('I1', 'Periode ' . date('d/m/Y', strtotime($sdate)) . ' - ' . date('d/m/Y', strtotime($edate)));
