...
class One_fin_006 extends RPT_Controller
{
    var $report_code;
    var $wQty;
    ...
    function index() {
        ...
        $this->pdf->setRptTitle('Laporan Usia Piutang');
        ...
        $r = $this->r_report->one_fin_006( isset($this->sys_input['customerid'])?$this->sys_input['customerid']:0 );
        ...
        if ($r)
        {
            ...
            //     }
                ...
                $this->pdf->Cell($w - 23.5, 0.7, $v['invoice_number'], 'LBR', 0, 'L', 0);
                $this->pdf->Cell(2, 0.7, $v['invoice_date'], 'LBR', 0, 'L', 0);
                $this->pdf->Cell(2, 0.7, $v['invoice_duedate'], 'LBR', 0, 'L', 0);
                $this->pdf->Cell(1, 0.7, $v['term_duration'], 'LBR', 0, 'C', 0);
                $this->pdf->Cell(2.5, 0.7, number_format($v['invoice_unpaid']), 'BR', 0, 'R', 0);
                
                $bjt = $v['date_diff'] <= 0 ? $v['invoice_unpaid'] : 0;
                $jt30 = $v['date_diff'] <= 30 && $v['date_diff'] > 0 ? $v['invoice_unpaid'] : 0;
                $jt60 = $v['date_diff'] <= 60 && $v['date_diff'] > 30 ? $v['invoice_unpaid'] : 0;
                $jt90 = $v['date_diff'] <= 90 && $v['date_diff'] > 60 ? $v['invoice_unpaid'] : 0;
                $jt120 = $v['date_diff'] <= 120 && $v['date_diff'] > 90 ? $v['invoice_unpaid'] : 0;
                $jtrest = $v['date_diff'] > 120 ? $v['invoice_unpaid'] : 0;
    ...
    function search()
    {
        ...
        $r = $this->r_reportfinance->fin_006($prm);
        $this->sys_ok($r);
    }
    ...
    function excel()
    {
        ...
        $filename = "laporan_usia_piutang_".date('d.m.Y', strtotime($this->input->get('sdate')))."_".date('d.m.Y', strtotime($this->input->get('edate'))).".xls";
        ...
        if ($r)
        {
            ...
            $objPHPExcel = $objReader->load("./assets/template_fin_006.xls");
