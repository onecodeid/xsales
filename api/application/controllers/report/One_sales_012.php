...
class One_sales_012 extends RPT_Controller
{
    var $report_code;
    var $wQty;
    ...
    function index() {
        ...
        // Get data
        $pcompany = "";
        if (isset($this->sys_input['iscompany'])) $pcompany = $this->sys_input['iscompany'];

        $this->load->model('report/r_report');
        $r = $this->r_report->one_sales_012( $this->sys_input['staffid'], $this->sys_input['sdate'], $this->sys_input['edate'], $pcompany);
        ...
        if ($r)
        {
            ...
            if (sizeof($d) > 0)
            {
                ...
                $is_company = $d[0]['customer_is_company'];
            }
            ...
            $this->tableHeader($this->pdf, ['staff_name' => $staff['staff_name'], 'sub_total' => $sub_total, 'sub_total_ppn' => $sub_total_ppn, 'is_company' => $is_company]);

            $this->pdf->SetFont('Arial','', 9);

            $wQty = $this->wQty;
            $wItemName = $this->pdf->w - $this->pdf->lMargin - $this->pdf->rMargin - 19.5;

            foreach ($d as $k => $v)
            {
                if ($staff_id != $v['staff_id'] || $is_company != $v['customer_is_company'])
                {
                    $staff_id = $v['staff_id'];
                    $is_company = $v['customer_is_company'];
                ...
                $this->pdf->Cell(1.5, 0.7, $b2b[$v['customer_is_company']], 'BR', 0, 'C', 0);
    ...
    {
        $wItemName = $me->w - $me->lMargin - $me->rMargin - 19.5;
        $this->pdf->SetFont('Arial','', 9);
        
        $me->SetFillColor(0,0,0);
        $me->SetTextColor(0,0,0);
        $me->Cell($me->w - $me->lMargin - $me->rMargin - 14, 1, "SALES : " . strtoupper($d['staff_name']) . " (".($d['is_company']=="Y"?"B2B":"B2C").")", '', 0, 'L', 0);
