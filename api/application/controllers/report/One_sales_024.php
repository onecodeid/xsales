...
class One_sales_024 extends RPT_Controller
{
    var $report_code;
    ...
    function index() {
        ...
        $r = $this->r_report->one_sales_024( $this->input->get('id'), $this->input->get('uid') );
        ...
        $page_cnt = sizeof($datas);
        $page_total = $page_cnt;
        if (sizeof($datas[$page_cnt-1])>8) { $datas[] = []; $page_total++; }
        ...
        foreach ($datas as $kkk => $vvv)
        {
            ...
            foreach ($data as $k => $v)
            {
                $bruto = $v['item_price']*$v['item_retur_qty'];
                $disc = $v['item_disc']*$bruto/100;
                
                $v['item_subtotal'] = $bruto - $disc;
                ...
                $this->pdf->Cell(1.5, $item_h, number_format($v['item_retur_qty'], 0), '', 0, 'R');
            ...
            if ($this->pdf->PageNo() == $page_total)
            {
                ...
                $this->pdf->MultiCell(6.8, 0.5, 'Terbilang : '.ucwords($this->terbilang($total - $r->sales_dp + $r->sales_shipping + $r->sales_ppn)) . ' Rupiah', '', 'L');
