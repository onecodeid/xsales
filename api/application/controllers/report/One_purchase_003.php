...
class One_purchase_003 extends RPT_Controller
{
    var $report_code;
    var $wQty;
    ...
    function search()
    {
        ...
        $r = $this->r_reportpurchase->purchase_003($prm);
        $this->sys_ok($r);
    }
    ...
    function excel()
    {
        ...
        $filename = "laporan_pembelian_per_vendor_".date('d.m.Y', strtotime($this->input->get('sdate')))."_".date('d.m.Y', strtotime($this->input->get('edate'))).".xls";
        ...
        if ($r)
        {
            ...
            $objPHPExcel = $objReader->load("./assets/template_purchase_003.xls");
            ...
            foreach ($r['records'] as $k => $v)
            {
                ...
                foreach ($v['bills'] as $l => $w)
                {
                    ...
                    $total[0] += $w->bill_subtotal;
                    $total[1] += $w->bill_disctotal;
                    $total[2] += $w->bill_ppn;
                    $total[3] += $w->bill_shipping;
                    $total[4] += $w->bill_grand_total;
                //     // $as->setCellValue('B'.$myLine, $w->bill_date);
                //     // $as->setCellValue('B'.$myLine, $w->bill_number);
                    $myLine++;
                }
