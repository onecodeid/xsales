...
class Budgeting extends MY_Controller
{
    function __construct()
    {
        ...
        $this->load->model('finance/f_budgeting');
    }

    function save()
    {   
    //     if (isset($this->sys_input['cash_id']))
        $r = $this->f_budgeting->save( $this->sys_input, $this->sys_user['user_id'] );
    //     else
    ...
    {
        $r = $this->f_budgeting->save_budget( $this->sys_input, $this->sys_user['user_id'] );      
        $this->sys_ok($r);
    }

    function copy_to()
    {
        $r = $this->f_budgeting->copy_to( $this->sys_input, $this->sys_user['user_id'] );      
        $this->sys_ok($r);
    }
    ...
    {
        $inp = $this->sys_input;
        $r = $this->f_budgeting->search(["search"=>"%", "sdate"=>$this->sys_input['sdate'], "edate"=>$this->sys_input['edate']]);
        $this->sys_ok($r);
    }
