...
class Billpay2 extends MY_Controller
{
    function __construct()
    {
        parent::__construct();

        $this->load->model('finance/f_billpay2');
    }

    // function search()
    // {
    //     $inp = $this->sys_input;
    //     $r = $this->f_bill->search([
    //         'search'=>'%'.$inp['search'].'%', 
    //         'page'=>$inp['page'], 
    //         'sdate'=>$inp['sdate'], 
    //         'edate'=>$inp['edate'],
    ...
    function search_id()
    {
        $r = $this->f_billpay2->search_id($this->sys_input['pay_id']);
        $this->sys_ok($r);
    }
    ...
    function delete()
    {
        if (isset($this->sys_input['memo_id']))
            $r = $this->f_bill->delete( $this->sys_input['memo_id'], $this->sys_user['user_id'] );
        else {
    ...
    function save()
    {
        ...
        if (isset($this->sys_input['pay_id']))
            $r = $this->f_billpay2->save( $this->sys_input, $this->sys_input['pay_id'], $this->sys_user['user_id'] );
        else
            $r = $this->f_billpay2->save( $this->sys_input, 0, $this->sys_user['user_id'] );
