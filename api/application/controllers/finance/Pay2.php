...
class Pay2 extends MY_Controller
{
    function __construct()
    {
        parent::__construct();

        $this->load->model('finance/f_pay2');
    }
    ...
    function delete()
    {
        if (isset($this->sys_input['pay_id']))
            $r = $this->f_pay2->delete( $this->sys_input['pay_id'], $this->sys_user['user_id'] );
        else {
    ...
    function save()
    {
        ...
        if (isset($this->sys_input['pay_id']))
            $r = $this->f_pay2->save( $this->sys_input, $this->sys_input['pay_id'], $this->sys_user['user_id'] );
        else
            $r = $this->f_pay2->save( $this->sys_input, 0, $this->sys_user['user_id'] );
