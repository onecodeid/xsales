...
class M_customer extends MY_Model
{
    ...
    function search( $d )
    {
        ...
        if ($r)
        {
            ...
            foreach ($r as $k => $v)
            {
                ...
                // discs
                $discs = $this->db->query("SELECT fn_master_customer_get_discs(?) x", [$v['customer_id']])->row();
                $r[$k]['discs'] = json_decode($discs->x);
            }
    ...
    function save ( $d )
    {
        $r = $this->db->query("CALL sp_master_customer_save(?,?,?,?,?,?)", [
            ...
            ]), isset($d['ddata'])?$d['ddata']:'[]', isset($d['bdata'])?$d['bdata']:'[]', isset($d['addresses'])?$d['addresses']:'[]', 0
        ])->row();
