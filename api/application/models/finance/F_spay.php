...
class F_spay extends MY_Model
{
    function __construct()
    {
        parent::__construct();

        $this->table_name = "f_spay";
        $this->table_key = "F_SpayID";
    }
    ...
    function save ( $d, $id = 0, $uid = 0)
    {
        ...
        $r = $this->db->query("CALL sp_finance_spay_save(?,?,?)", [
                        $id, 
                        $d['hdata'],
                        $uid
                    ])
    ...
    {
        $r = $this->db->query("CALL sp_finance_spay_delete(?, ?)", [
            $id, 
            $uid
        ])
        ->row();
