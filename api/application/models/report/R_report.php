...
class R_report extends MY_Model
{
    ...
    function One_sales_013($sdate, $edate)
    {
        $r = $this->db->query("CALL `sp_r_ONE-SAL-013`(?, ?)", [$sdate, $edate])
                    ->result_array();
        $this->clean_mysqli_connection($this->db->conn_id);
        return $r;
    }
    ...
    // Report Penjualan Per Sales Per Tanggal Detail Item
    function One_sales_022($sdate, $edate, $customer = 0)
    {
        $r = $this->db->query("CALL `sp_r_ONE-SAL-022`(?, ?, ?)", [$sdate, $edate, $customer])
                    ->result_array();
        $this->clean_mysqli_connection($this->db->conn_id);
        return $r;
    }
    ...
    // Report Nota Invoice
    function One_sales_023($id, $uid)
    {
        $r = $this->db->query("CALL `sp_r_ONE-SAL-023`(?, ?)", [$id, $uid])
                    ->row();
        $this->clean_mysqli_connection($this->db->conn_id);
        return $r;
    }
