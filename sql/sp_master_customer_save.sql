...
DELIMITER ;;
CREATE PROCEDURE `sp_master_customer_save` (IN `customerid` int, IN `jdata` mediumtext, IN `ddata` mediumtext, IN `bdata` mediumtext, IN `addresses` mediumtext, IN `uid` int)
BEGIN
...
UPDATE m_customerdisc
SET M_CustomerDiscIsActive = "O"
WHERE M_CustomerDiscM_CustomerID = customerid
AND M_CustomerDiscIsActive = "Y";

SET l = JSON_LENGTH(ddata);
SET n = 0;
WHILE n < l DO

    SET tmp = JSON_EXTRACT(ddata, CONCAT("$[", n, "]"));
    SET d_disc = JSON_UNQUOTE(JSON_EXTRACT(tmp, "$.disc"));
    SET d_amount = JSON_UNQUOTE(JSON_EXTRACT(tmp, "$.amount"));
    
    SET disc_id = (SELECT M_CustomerDiscID FROM m_customerdisc WHERE M_CustomerDiscM_CustomerID = customerid
                AND M_CustomerDiscIsActive = "O"
                AND M_CustomerDiscM_DiscID = d_disc);
    IF disc_id IS NULL THEN
        INSERT INTO m_customerdisc(M_CustomerDiscM_CustomerID, M_CustomerDiscM_DiscID, M_CustomerDiscAmount)
        SELECT customerid, d_disc, d_amount;
    ELSE
        UPDATE m_customerdisc
        SET M_CustomerDiscAmount = d_amount, M_CustomerDiscIsActive = "Y"
        WHERE M_CustomerDiscID = disc_id;
    END IF;
