...
IF `what` = 'SALES' THEN
    ...
    SET invoice_total = 
        (SELECT JSON_OBJECT('total_unpaid', SUM(IF(L_InvoiceDueDate>=DATE(very_start_date), L_InvoiceUnpaid, 0)),
                            'total_due', SUM(IF(L_InvoiceDueDate<DATE(now()) AND L_InvoiceDate>=DATE(very_start_date), L_InvoiceUnpaid, 0)),
