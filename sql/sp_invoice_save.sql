...
DECLARE invoice_paid DOUBLE DEFAULT 0;
...
rcvloop : WHILE n < l Do
    ...
    IF cinvoice_id <> 0 AND cinvoice_id <> invoice_id THEN
...
IF success = 1 THEN
    IF invoice_id <> 0 THEN
        SET invoice_paid = (SELECT L_InvoicePaid FROM l_invoice WHERE L_InvoiceID = invoice_id);
    END IF;

    IF invoice_paid > 0 THEN
        SELECT "ERR" status, "Invoice sudah dibayar, tidak bisa mengubah data :(" message;
        ROLLBACK;
    ELSE
