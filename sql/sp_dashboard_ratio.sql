DROP PROCEDURE `sp_dashboard_ratio`;
DELIMITER ;;
CREATE PROCEDURE `sp_dashboard_ratio` (IN `sdate` date, IN `edate` date, IN `thetype` varchar(50))
BEGIN
...
SELECT *, (activa * 100 / liability) rst FROM (
SELECT 
SUM(IF(report_type=thetop, journal_balance, 0)) activa,
SUM(IF(report_type=thebottom, journal_balance, 0)) liability
...
WHERE j_debit > 0 OR j_credit > 0) xxyy) zz;
