DROP PROCEDURE `sp_purchase_receive_save`;
DELIMITER ;;
CREATE PROCEDURE `sp_purchase_receive_save` (IN `pid` int, IN `hdata` varchar(2000), IN `jdata` text, IN `uid` int)
BEGIN
...
ELSE
    ...
    IF pid = 0 THEN
        ...
        INSERT INTO p_receive(P_ReceiveDate,
            ...
            P_ReceiveMemo,
            P_ReceiveUID)
        SELECT pdate, pnumber, prefnumber, pvendor, pwarehouse, ptotal, pnote, pmemo, uid;

        SET pid = (SELECT LAST_INSERT_ID());
        CALL sp_log_activity("CREATE", "PURCHASE.RECEIVE", pid, uid);
    ELSE

        UPDATE p_receive
        SET P_ReceiveDate = pdate, P_ReceiveNote = pnote, P_ReceiveMemo = pmemo, P_ReceiveRefNumber = prefnumber
        WHERE P_ReceiveID = pid;
        CALL sp_log_activity("MODIFY", "PURCHASE.RECEIVE", pid, uid);
    END IF;
    ...
    WHILE n < l DO
        ...
        Set prid = (Select P_PurchaseDetailP_PurchaseID from p_purchasedetail where P_PurchaseDetailID = d_purchase);
