...
IF transfer_date IS NULL THEN SET transfer_date = date(now()); END IF;
...
IF transfer_id = 0 THEN
	...
	SELECT fn_numbering('TRF'), transfer_date, warehouse_id, to_warehouse_id, transfer_note;
...
WHILE i < l DO
	...
	IF detail_id IS NULL THEN
        ...
        UPDATE i_stock
			...
			I_StockLastTransDate = IF(transfer_date = date(now()), now(), CONCAT(transfer_date, " 23:59:59") ),
