...
SET account_disc_id = (SELECT fn_master_get_account_id("ACC.PURCHASE.DISC"));
SET account_revenue_id = (SELECT fn_master_get_account_id("ACC.EXPENSE.OTHER"));
SET account_retur_id = (SELECT fn_master_get_account_id("ACC.PURCHASE.RETUR"));
...
SET adata = CONCAT(JSON_OBJECT("account", account_id, "credit", total+total_change, "debit", 0,
            "ledger_note", JSON_ARRAY(account_payable_name, supplier_name)),
			",", JSON_OBJECT("account", account_payable_id, "credit", 0, "debit", total+total_disc+total_retur+total_dp, "ledger_note", JSON_ARRAY(account_name, supplier_name)));
...
IF total_change <> 0 OR total_change_disc <> 0 OR total_change_retur <> 0 OR total_change_dp <> 0 THEN
	SET adata = CONCAT(adata, ",", JSON_OBJECT("account", account_revenue_id, "credit", 0, "debit", total_change+total_change_retur+total_change_disc+total_change_dp,
            "ledger_note", JSON_ARRAY(account_name, supplier_name)));
END IF;
