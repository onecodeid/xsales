...
SET staff_name = (SELECT CONCAT(S_StaffFirstName, " ", S_StaffLastName) FROM one_iv.a_customer 
	JOIN one_iv.s_staff ON A_CustomerS_StaffID = S_StaffID
	WHERE A_CustomerID = payment_customer);
...
ELSE
    ...
    JOIN (
        ...
        FROM f_payment2detail
        WHERE F_Payment2DetailIsActive = "O" AND F_Payment2DetailF_Payment2ID = paymentid AND F_Payment2DetailIsRetur = "Y" AND F_Payment2DetailL_SalesReturID <> 0
...
WHILE n < l DO
	...
	SET d_id = (SELECT F_Payment2DetailID FROM f_payment2detail WHERE F_Payment2DetailIsActive = "O" AND F_Payment2DetailF_Payment2ID = paymentid AND F_Payment2DetailL_InvoiceID = d_invoice AND F_Payment2DetailIsDisc = d_disc AND F_Payment2DetailIsRetur = d_is_retur AND F_Payment2DetailL_SalesReturID = d_retur_id);
	-- SELECT d_id, paymentid, d_invoice, d_disc, d_is_retur,d_retur_id, d_amount;
    -- NEW RECORD
    -- -
	IF d_id IS NULL THEN
        SET d_unpaid = (SELECT L_InvoiceUnpaid FROM one_iv.l_invoice WHERE L_InvoiceID = d_invoice);
		...
		INSERT INTO f_payment2detail(F_Payment2DetailF_Payment2ID, F_Payment2DetailL_InvoiceID, F_Payment2DetailAmount, F_Payment2DetailChange, F_Payment2DetailIsDisc, F_Payment2DetailIsRetur, F_Payment2DetailL_SalesReturID)
	...
	ELSE
		...
		UPDATE f_payment2detail
		SET F_Payment2DetailAmount = d_amount + d_x_change, F_Payment2DetailChange = d_x_change, F_Payment2DetailIsDisc = d_disc, F_Payment2DetailIsRetur = d_is_retur, F_Payment2DetailL_SalesReturID = d_retur_id, F_Payment2DetailIsActive = "Y"
    ...
    IF d_retur_id <> 0 THEN
        UPDATE one_iv.l_salesretur SET L_SalesReturUsed = L_SalesReturUsed + d_amount - o_amount WHERE L_SalesReturID = d_retur_id;
    END IF;
...
WHILE n < l DO
    ...
    SELECT F_Payment2DetailL_InvoiceID, F_Payment2DetailIsRetur, F_Payment2DetailIsDisc,F_Payment2DetailL_SalesReturID, F_Payment2DetailAmount,
...
UPDATE t_journal SET T_JournalRefNote = customer_name, T_JournalRefNote2 = staff_name WHERE T_JournalID = journal_id;
